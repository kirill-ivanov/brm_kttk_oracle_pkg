CREATE OR REPLACE PACKAGE PK30_BILLING_SPB
IS
    --
    -- Пакет для поддержки процесса выставления счетов и закрытия периода
    -- биллинга СПб-Порт-телеком (Барсум)
    -- Закрываются только счета стоящие в очереди: BILLING_QUEUE_T
    --
    -- ==============================================================================
    c_PkgName   constant varchar2(30) := 'PK30_BILLING_SPB';
    -- ==============================================================================
    -- В Ы Б О Р   Б И Л Л И Н Г А
    --
    c_TASK_ID   constant integer := Pk00_Const.c_BILLING_SPB;
    -- ------------------------------------------------------------------------- --
...
END PK30_BILLING_SPB;
/
CREATE OR REPLACE PACKAGE BODY PK30_BILLING_SPB
IS
      ...
      FROM (
        SELECT B.BILL_ID, B.ACCOUNT_ID, A.BILLING_ID, AP.PROFILE_ID, 
         ...
         WHERE B.REP_PERIOD_ID = p_period_id
           ...
           AND A.BILLING_ID    = Pk00_Const.c_BILLING_SPB
...
BEGIN
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Добавить региональный префикс к именам счетов
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    UPDATE BILL_T B 
       SET B.BILL_NO = '7801/'||B.BILL_NO 
     WHERE B.REP_PERIOD_ID = p_period_id
       AND B.BILL_NO NOT LIKE '7801/%'
       AND B.ACCOUNT_ID IN (
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- поставить в очередь на выставление счетов клиентов юр.лиц С-Петербурга
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    PK30_BILLING_BASE.Correct_tax_incl( p_period_id );
