    ...
    OPEN p_recordset FOR
        ...
        WITH BDR AS (
            SELECT --+ parallel(b 10)
             ...
             GROUP BY account_id, item_id, trf_type
        ), IT AS (
            SELECT '`'||C.CONTRACT_NO CONTRACT_NO, CL.CLIENT_NAME, 
              ...
              FROM ACCOUNT_T A, BILL_T B, ITEM_T I, CONTRACT_T C, CLIENT_T CL, SERVICE_T S, SUBSERVICE_T SS
        ...
        ORDER BY IT.CONTRACT_NO, IT.CLIENT_NAME, IT.BILL_NO, IT.SERVICE
        ;
EXCEPTION
    WHEN OTHERS THEN
        v_retcode := Pk01_SysLog.Fn_write_Error('ERROR', c_PkgName||'.'||v_prcName);
...
BEGIN
    ...
    OPEN p_recordset FOR
        WITH bdr AS ( 
            SELECT --+ parallel(b 10) 
                  account_id, bill_id, order_id, trf_type, service_id, subservice_id, parent_subsrv_id, 
             ...
             GROUP BY account_id, bill_id, order_id, trf_type, service_id, subservice_id, parent_subsrv_id 
        ) 
        SELECT  DISTINCT 
                '`'||c.CONTRACT_NO CONTRACT_NO, CL.CLIENT_NAME, S.SERVICE, 
                ...
                bdr.first_call, 
                bdr.last_call 
          FROM bdr, X07_SRV_DCT ss, SERVICE_T S, SUBSERVICE_T SR, BILL_T B, CONTRACT_T C, CLIENT_T CL 
        ...
        ORDER BY CONTRACT_NO, CL.CLIENT_NAME, S.SERVICE
        ;
EXCEPTION
    WHEN OTHERS THEN
        v_retcode := Pk01_SysLog.Fn_write_Error('ERROR', c_PkgName||'.'||v_prcName);
