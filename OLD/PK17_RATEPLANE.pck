...
IS
    ...
    FUNCTION Update_rateplan(
                   ...
                   p_tax_incl       IN CHAR,     -- Налоги включены в ТП: "Y/N"
                   p_rateplan_name  IN VARCHAR2, -- имя тарифного плана
                   p_rateplan_code  IN VARCHAR2  -- код тарифного плана 
               ) RETURN INTEGER;
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- добавить строку в помесячный тариф на абонплату для бюджетников
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    FUNCTION Add_month_tariff(
                   p_rateplan_id IN INTEGER, -- ID тарифного планы в системе ведения тарифов
                   p_period_id   IN INTEGER, -- ID периода, для которого установлен тариф
                   p_price       IN NUMBER   -- цена за указанный период
               ) RETURN INTEGER;             -- Возвращает ID строки тарифа
...
BEGIN
    ...
    update RATEPLAN_T rp
        set 
            rp.RATEPLAN_NAME   = p_rateplan_name,
            rp.TAX_INCL        = p_tax_incl,
            rp.rateplan_code   = p_rateplan_code
    where  
...
IS
    ...
    v_tariff_id  INTEGER;
BEGIN
    -- получаем id-строки тарифа
    v_tariff_id := SQ_MONTH_TARIFF_ID.NEXTVAL;
    -- сохраняем строку тарифа за указанный месяц
    INSERT INTO MONTH_TARIFF_T (
        TARIFF_ID, RATEPLAN_ID, PERIOD_ID, PRICE, CREATE_DATE
    )VALUES (
        v_tariff_id, p_rateplan_id, p_period_id, p_price, SYSDATE
    );
    RETURN v_tariff_id;
