...
IS
    ...
    FUNCTION New_bill_item (
                   ...
                   p_order_body_id  IN INTEGER,   -- ID строки компонента услуги в заказе
...
-- произвести корректировку суммы позиции счета
--   - при ошибке выставляет исключение
PROCEDURE Edit_bill_adjust_item (
               p_bill_id       IN INTEGER,   -- ID счета
...
IS
    v_prcName     CONSTANT VARCHAR2(30) := 'New_bill_item';
    v_item_id     INTEGER;
...
BEGIN
    ...
    INSERT INTO ITEM_T (
       ...
       p_order_id, p_service_id, p_order_body_id, p_subservice_id, p_charge_type,
       p_item_total, 0,
...
IS
    v_prcName    CONSTANT VARCHAR2(30) := 'NEW_BILL_ADJUST_ITEM';
    v_item_id    INTEGER;
    v_item_type  ITEM_T.ITEM_TYPE%TYPE;
...
BEGIN
    ...
    BEGIN
        --Ищем позицию счета и тип
        SELECT ORDER_BODY_ID, CHARGE_TYPE 
               INTO v_order_body_id, v_charge_type 
           FROM ORDER_BODY_T
        WHERE ORDER_ID = p_order_Id
              and SUBSERVICE_ID = p_subservice_id
              AND p_date_from between DATE_FROM AND NVL(DATE_TO,TO_DATE('01.01.2050','DD.MM.YYYY'));
        
       --Для скидки (если находим ее) ставим - одноразовый тип услуги
       IF p_subservice_Id = Pk00_CONST.c_SUBSRV_DISC THEN
          v_charge_type := Pk00_CONST.c_CHARGE_TYPE_ONT;
       END IF; 
    EXCEPTION
       WHEN NO_DATA_FOUND THEN
          -- Не найти может только в том случае, если это скидка, а у заказа этого не было
          IF p_subservice_Id = Pk00_CONST.c_SUBSRV_DISC THEN
              v_order_body_id := PK06_ORDER.Add_subservice (
                       p_order_Id,
                       p_subservice_id,                                        
                       Pk00_CONST.c_CHARGE_TYPE_ONT,
                       NULL,                       
                       TRUNC(SYSDATE),
    ...
    INSERT INTO ITEM_T (
       ...
       p_bill_id, p_rep_period_id, v_item_id, v_item_type,
       p_order_id, v_order_body_id, p_service_id, p_subservice_id, v_charge_type,
       p_adjusted, 0, Pk00_Const.c_ITEM_STATE_OPEN,
       p_date_from, p_date_to,
...
IS
    v_prcName    CONSTANT VARCHAR2(30) := 'EDIT_BILL_ADJUST_ITEM';
...
BEGIN
    ...
    UPDATE ITEM_T i
       SET 
           i.ITEM_TOTAL =  v_value,
           i.TAX_INCL = DECODE(p_tax_include,1,'Y',0,'N'),
           i.DATE_FROM = p_date_from,
           i.DATE_TO = p_date_to,
           i.DESCR = p_notes,
           LAST_MODIFIED = SYSDATE
     WHERE BILL_ID = p_bill_id
...
BEGIN
    ...
    -- Сначала удаляем детализацию, если она на ней есть
    ...
    BEGIN
       ...
       SELECT 
           BDR_TABLE INTO v_bdr_table
       FROM BDR_TYPES_T
         WHERE BDR_TYPE_ID = v_external_id;
          
       v_sql := 'DELETE FROM '|| v_bdr_table ||
            ' WHERE bill_id = '|| p_bill_id ||
                  ' AND item_id = ' || p_item_id ||
                  ' AND rep_period BETWEEN TO_DATE ('|| p_rep_period_id ||', ''YYYYMM'')
                                      AND   LAST_DAY (TO_DATE ('|| p_rep_period_id ||', ''YYYYMM'')) + INTERVAL ''00 23:59:59'' DAY TO SECOND';         
