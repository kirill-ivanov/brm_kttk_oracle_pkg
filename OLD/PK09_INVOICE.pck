...
BEGIN    
    ...
    ELSIF p_tax_incl = PK00_CONST.c_RATEPLAN_TAX_NOT_INCL THEN -- налог не включен
        RETURN p_amount + PK09_INVOICE.Calc_tax(p_amount, p_vat);
...
BEGIN    
    IF p_tax_incl = PK00_CONST.c_RATEPLAN_TAX_INCL THEN      -- налог включен в начисленную сумму
        RETURN p_amount - PK09_INVOICE.Allocate_tax(p_amount, p_vat);
    ELSIF p_tax_incl = PK00_CONST.c_RATEPLAN_TAX_NOT_INCL THEN -- налог не включен
        RETURN p_amount;
    ELSE -- защита от ошибок
        RETURN NULL;
...
/*
                   А  Р  Х  И  В
-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
-- Расчитать позицию счета-фактуры, для непереодического счета,
-- item-ы распределяются на invoice_item-ы в зависимости от даты оказания услуги 
    --   - положительное - ID invoice_item, 
    --   - при ошибке выставляет исключение
PROCEDURE Calc_inv_item (
               p_inv_item_no   IN OUT INTEGER, -- номер строки в счете фактуре
               p_bill_id       IN INTEGER,   -- ID позиции счета
               p_rep_period_id IN INTEGER,   -- ID отчетного периода счета
               p_service_id    IN INTEGER,   -- ID услуги
...
BEGIN
    ...
    SELECT 
         ...
         SUM(TOTAL), SUM(GROSS), SUM(TAX), MIN(DATE_FROM), MAX(DATE_TO)
      FROM (
        SELECT  
           -- Полная сумма начислений с налогами -------------
           CASE
...
BEGIN
    ...
    THEN
        ...
        OPEN c_items FOR
          SELECT SERVICE_ID, 
                 TRUNC(DATE_FROM,'mm') DATE_FROM 
            FROM ITEM_T
           WHERE BILL_ID = p_bill_id
             AND REP_PERIOD_ID = p_rep_period_id
             AND SERVICE_ID IS NOT NULL
             GROUP BY SERVICE_ID, TRUNC(DATE_FROM,'mm')
             ORDER BY 1,2
        ;
    ELSE 
         -- все item-ы группируются в один invoice_item 
    ...
    --
    LOOP
        FETCH c_items INTO v_service_id, v_date_from;
