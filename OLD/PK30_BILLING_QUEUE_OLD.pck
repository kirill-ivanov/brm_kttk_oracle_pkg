CREATE OR REPLACE PACKAGE PK30_BILLING_QUEUE_OLD
IS
    --
...
END PK30_BILLING_QUEUE_OLD;
/
CREATE OR REPLACE PACKAGE BODY PK30_BILLING_QUEUE_OLD
IS
...
BEGIN
    ...
    SELECT BILL_ID, ACCOUNT_ID, BILLING_ID, PROFILE_ID, p_task_id, REP_PERIOD_ID
      ...
      WHERE DATE_FROM = MAX_DATE_FROM -- используем последний открытый в периоде профиль
...
BEGIN
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Удаляем позиции фиксированных начислений, за исключением тех что 
    -- сформировал тарификатор трафика
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
