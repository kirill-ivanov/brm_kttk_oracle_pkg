CREATE OR REPLACE PACKAGE PK51_W_ACCOUNT_EXT_B
IS
    --
...
--================================================================================================================
-- Получение доп. информации по  заказу (минималка, абон. плата)
    PROCEDURE ORDER_EXT_PARAM( 
         p_result        OUT VARCHAR2, 
         p_recordset     OUT t_refc, 
         p_order_id      IN NUMBER    -- номер заказа
    );    
...
END PK51_W_ACCOUNT_EXT_B;
/
CREATE OR REPLACE PACKAGE BODY PK51_W_ACCOUNT_EXT_B
IS
         ...
         WITH OB_RATE_LEVEL AS (
             SELECT ob.order_body_id, 
               ...
               FROM
                    ...
                    dictionary_t dict
              WHERE 
                 dict.key_id = ob.RATE_LEVEL_ID
                 AND dict.parent_id = 23      
                 AND ob.rate_level_id = 2301
         )
         SELECT 
                 OB.ORDER_BODY_ID,
                 OB.ORDER_ID,
                 ...
                 DICT.NAME CHARGE_TYPE,
	               ob.date_from,
                 OB.RATEPLAN_ID,
                 RATE_VALUE,      -- минималка/абонка
                 OB.RATE_RULE_ID,
                 dict_rate_rule.name RATE_RULE_NAME,
                 OB.RATE_LEVEL_ID,
                 OB_RATE_LEVEL.name RATE_LEVEL_NAME,
                 TRF.RATEPLAN_NAME,
                 TRF.RATESYSTEM_ID RATEPLAN_SYSTEM_ID,
                 TRF.RATEPLAN_CODE RATEPLAN_CODE,
                 OB.CURRENCY_ID,
           ...
           WHERE o.order_id = ob.ORDER_ID
                 ...
                 AND ob.order_body_id = OB_RATE_LEVEL.order_body_id (+)
...
IS
    v_prcName            CONSTANT VARCHAR2(30) := 'ORDER_BODY2';
    v_retcode            INTEGER;
begin
    open p_recordset for
         SELECT 
...
IS
    v_prcName            CONSTANT VARCHAR2(30) := 'ORDER_BODY_CHANEL';
    v_retcode            INTEGER;
begin
    open p_recordset for
      WITH OB_RATE_LEVEL AS (
               SELECT ob.order_body_id, 
                ...
                WHERE 
                   dict.key_id = ob.RATE_LEVEL_ID
                   AND dict.parent_id = 23      
                   AND ob.rate_level_id IN (2301,2302)
      )  
       SELECT 
                 OB.ORDER_BODY_ID,
                 OB.ORDER_ID,
                 ...
                 NULL SPEED_VALUE,
                 NULL SPEED_UNIT_ID,
                 ip.speed_str SPEED_UNIT_NAME,
                 OB.QUANTITY,
                 OB.RATE_VALUE, 
                 OB.CURRENCY_ID,
                 OB_RATE_LEVEL.rate_level_id,
                 OB_RATE_LEVEL.name rate_level_NAME                 
            FROM order_t o, 
                 order_body_t ob, 
                 subservice_t ss,
...
begin
    open p_recordset for
         SELECT f.fix_rate_id,
             f.charge_type,
             d.name charge_name,
             f.VALUE
        FROM FIX_RATE_T F, DICTIONARY_T D
       WHERE     F.order_Id = p_order_id
             AND d.key(+) = f.charge_type
             AND d.parent_id(+) = 7 ;
exception
   WHEN OTHERS THEN
        v_retcode := Pk01_SysLog.Fn_write_Error('ERROR', c_PkgName||'.'||v_prcName);
    ...
    IF p_account_id IS NOT NULL THEN    
        open p_recordset for
            SELECT op.*
             ...
             WHERE op.order_id = ord.order_id
                   AND account_id = p_account_id
                   AND (op.date_from <p_date_to AND op.date_to > p_date_from)
                   ;
     ELSE
        open p_recordset for
             SELECT *
