...
BEGIN
    INSERT INTO CONTRACT_T (
      ...
      SQ_CONTRACT_ID.NEXTVAL, p_contract_no, p_date_from, p_date_to, p_client_id
    )
...
BEGIN
    INSERT INTO CONTRACT_T (
      CONTRACT_ID, CONTRACT_NO, DATE_FROM, DATE_TO, CLIENT_ID, 
      MARKET_SEGMENT_ID, CLIENT_TYPE_ID,CONTRACT_TYPE_ID, NOTES
    )VALUES(
      SQ_CONTRACT_ID.NEXTVAL, p_contract_no, p_date_from, p_date_to, p_client_id, 
      p_market_segment_id, p_client_type_id, p_contract_type_id, p_notes
    )
    RETURNING CONTRACT_ID INTO v_contract_id;
    -- привязываем менеджера к договору
    IF p_manager_id IS NOT NULL THEN
