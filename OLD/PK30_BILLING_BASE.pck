...
BEGIN
    ...
    INSERT INTO BILLING_QUEUE_T (
        BILL_ID, ACCOUNT_ID, BILLING_ID, PROFILE_ID, TASK_ID, REP_PERIOD_ID, DATA_PERIOD_ID
    )
...
--        ),
    ...
    UPDATE BILL_T B
       SET B.PAID_TO   = NULL, 
           B.DUE_DATE  = SYSDATE, 
           B.CALC_DATE = SYSDATE
     WHERE B.BILL_STATUS = Pk00_Const.c_BILL_STATE_EMPTY
       AND EXISTS (
          SELECT * FROM BILLING_QUEUE_T Q
           WHERE Q.REP_PERIOD_ID = B.REP_PERIOD_ID
             AND Q.BILL_ID = B.BILL_ID
...
-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
-- Рассчитать оборты за период для всех лицевых счетов, где были высталены СЧЕТА за месяц
-- В очередь BILLING_QUEUE_T можем поставить только счет, 
-- но возможен вариант для биллинга 2003, когда клиент только платил за месяц, 
                 ...
                 WHERE EXISTS ( -- формально может быть более одной записи с account_id
                       SELECT * FROM BILLING_QUEUE_T Q
                        WHERE Q.ACCOUNT_ID    = P.ACCOUNT_ID
                          AND Q.REP_PERIOD_ID = P.REP_PERIOD_ID
                          AND Q.TASK_ID       = p_task_id
                   )
            )
        ...
        PREV_PERIOD AS (
            ...
            FROM (
                SELECT ACCOUNT_ID, 
                 ...
                 WHERE EXISTS ( -- формально может быть более одной записи с account_id
                       SELECT * FROM BILLING_QUEUE_T Q
                        WHERE Q.ACCOUNT_ID = R.ACCOUNT_ID
                          AND Q.TASK_ID    = p_task_id
                   )
              ...
              FROM (
                SELECT PI.ACCOUNT_ID, PI.REP_PERIOD_ID, PI.CLOSE_BALANCE, 
                 ...
                 WHERE PI.REP_PERIOD_ID <= v_last_period_id
                   AND EXISTS (
                       SELECT * FROM BILLING_QUEUE_T Q
                        WHERE Q.ACCOUNT_ID = PI.ACCOUNT_ID
                          AND Q.TASK_ID = p_task_id
                   )
            ) WHERE REP_PERIOD_ID = MAX_PERIOD_ID
        ) 
...
BEGIN
    ...
    USING (       
        SELECT B.ACCOUNT_ID, CR.REGION_ID, B.BILL_ID, B.REP_PERIOD_ID, B.BILL_NO 
         ...
         WHERE B.CONTRACTOR_ID = CR.CONTRACTOR_ID
