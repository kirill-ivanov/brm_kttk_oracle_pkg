...
--    c_LOAD_CODE_OK        CONSTANT INTEGER := 2;    -- ОК
    ...
    --============================================================================================
    -- Добавить ROUTER_ZONE на заказ VPN
    --============================================================================================
    PROCEDURE Load_vpn_router_zone;
...
BEGIN
    ...
    SELECT CONTRACT_NO, ACCOUNT_NO, COMPANY
      FROM (
        select  
      ...
      WHERE 1=1
        AND ACCOUNT_NO in ('MS000655','MS000895','MS006485','MS006854')
...
BEGIN
    ...
    v_count := SQL%ROWCOUNT;
    Pk01_Syslog.Write_msg('Недопустимый тип услуги - '||v_count, c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );                                 
    ...
    UPDATE PK213_PINDB_ORDERS_T XO
     ...
     WHERE XO.SERVICE_NAME IN (
       ...
       AND XO.LOAD_CODE IS NULL;
    v_count := SQL%ROWCOUNT;
    Pk01_Syslog.Write_msg('Недопустимые услуги - '||v_count, c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );
    ...
    USING (
        SELECT DISTINCT X.ACCOUNT_ID, X.BRM_ACCOUNT_ID
          FROM PK213_PINDB_ALL_CONTRACTS_T X
         WHERE X.LOAD_CODE IN ( c_LOAD_CODE_START, 8 )
    ) X
    ON (
    ...
    FOR r_ord IN (
      SELECT DISTINCT  XO.ORDER_NO, X.BRM_ACCOUNT_NO, XO.ACCOUNT_POID
       ...
       WHERE XO.LOAD_CODE IS NULL
         ...
         AND X.LOAD_CODE IN ( c_LOAD_CODE_START, 8 )
    )
    LOOP
...
BEGIN
    ...
    v_count := SQL%ROWCOUNT;
    Pk01_Syslog.Write_msg('Mark MultiPart 8-800 '||v_count||' - rows set', c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );
    ...
    DELETE FROM PK213_PINDB_ORDERS_T XO
     WHERE XO.BRM_SERVICE_ID = 2 
       ...
       AND XO.LOAD_CODE IS NULL;
    v_count := SQL%ROWCOUNT;
    Pk01_Syslog.Write_msg('Mark MultiPart 8-800 '||v_count||' - rows deleted tmp', c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );
...
PROCEDURE Load_vpn_router_zone
IS
    v_prcName         CONSTANT VARCHAR2(30) := 'Load_vpn_router_zone';
    v_count           INTEGER := 0;
    v_order_body_id   INTEGER;
BEGIN
    Pk01_Syslog.Write_msg('Start', c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );
    ...
    FOR rec IN (
      SELECT 
            o.BRM_ORDER_ID, 
            ob_o.zone 
        FROM PK213_PINDB_ORDERS_T o,
             ccad.orders@PINDB.WORLD ob_o 
       WHERE 
             o.BRM_RATERULE_ID = 2408
         AND o.LOAD_CODE != -1
         and ob_o.order_code = o.BRM_ORDER_NO
    )
    LOOP
        update order_info_t oi
          set OI.ROUTER_ZONE = rec.zone
          where OI.ORDER_ID = rec.BRM_ORDER_ID
          ; 
        
        v_count := v_count + 1;
    END LOOP;
    
    Pk01_Syslog.Write_msg(v_count||' - rows updated', c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );
    -- - - - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - --
    Pk01_Syslog.Write_msg('Stop', c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );
EXCEPTION
    WHEN OTHERS THEN
