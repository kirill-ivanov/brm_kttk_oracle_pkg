...
IS
    ...
    c_BILLING_DISC   CONSTANT INTEGER := 2097; -- Тестовый биллинг КСАД
    ...
    --============================================================================================
    -- Процедура проверки соответствия данных PINDB и BRM.AG_IMPORT_NPL_T
    --============================================================================================
    ...
    -- Откат информации о заказах
    PROCEDURE Rollback_orders;
    
    -- Откат информации о договоре
    PROCEDURE Rollback_contracts;
    
    -- Откат информации о лицевых счетах
...
--============================================================================================
-- После полного тестирования переводим л/с из c_BILLING_DISC -> Pk00_Const.c_BILLING_OLD
--============================================================================================
PROCEDURE Change_billing_id
IS
...
BEGIN
    ...
    -- проставляем BRANCH_ID, который уже есть в BRM:
    ...
    -- проставляем CLIENT_ID, который уже есть в BRM:
    MERGE INTO AG_IMPORT_NPL_T N
    USING (
    ...
    -- проставляем CUSTOMER_ID, который уже есть в BRM:
    MERGE INTO AG_IMPORT_NPL_T N
    USING (
    ...
    -- проставляем CONTRACT_ID, который уже есть в BRM:
    MERGE INTO AG_IMPORT_NPL_T N
    USING (
    ...
    -- проставляем ACCOUNT_ID, который уже есть в BRM:
    MERGE INTO AG_IMPORT_NPL_T N
    USING (
    ...
    -- проставляем ORDER_ID, который уже есть в BRM:
    MERGE INTO AG_IMPORT_NPL_T N
    USING (
    ...
    -- Собираем статистику по AG_IMPORT_NPL_T
...
BEGIN
    ...
    where a.gl_segment not like '%RP%' 
      ...
      and ap.status = 1 -- К.Федоров считает это ограничение лищ=шним
    ...
    DELETE FROM PINDB_NPL_ORDERS_T PO
     WHERE PO.ORDER_NO NOT IN (
        ...
        --'09K032831/17', -- 
        --'07K2751/14',
        -- 2)  IP-access - тарифицируемые по объему.
        '2/4K013555/CON'
        -- 3)  BURST (это у меня есть в явном виде)
        --'15K003248',
        --'15K003252',
        --'15K003464',
        --'15K003557',
...
BEGIN
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    UPDATE PINDB_NPL_ORDERS_T PO SET PO.LOAD_STATUS = c_LDSTAT_DBL_ORD -- заказ уже есть в BRM
    WHERE EXISTS (
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    UPDATE PINDB_NPL_ORDERS_T PO SET PO.LOAD_STATUS = c_LDSTAT_NOT_SRV -- услуга не найдена
    WHERE NOT EXISTS (
    ...
    USING (
      SELECT
        ...
        FROM PINDB_NPL_ORDERS_T PO, ORDER_T O, DICTIONARY_T D 
       WHERE O.ORDER_NO = PO.ORDER_NO
         AND D.PARENT_ID(+) = 67
    ...
    WHEN MATCHED THEN UPDATE SET 
             ...
             SPEED_UNIT_ID = SPEED_UNIT_ID
    WHEN NOT MATCHED THEN INSERT (
             OI.ORDER_ID, OI.POINT_SRC, OI.POINT_DST, 
             OI.SPEED_STR, OI.ROUTER_ZONE, OI.SPEED_VALUE, OI.SPEED_UNIT_ID)
          VALUES (
             PO.ORDER_ID, PO.POINT_SRC, PO.POINT_DST, 
             PO.SPEED_STR, PO.ROUTER_ZONE, PO.SPEED_VALUE, PO.SPEED_UNIT_ID
          );
    /*
...
--============================================================================================
-- Откат ВСЕХ начислений и счетов
--============================================================================================
PROCEDURE Rollback_bills
IS
...
BEGIN
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    -- удаляем дополнительную информацию об услугах предоставления каналов связи
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    DELETE FROM ORDER_INFO_T I
     WHERE EXISTS (
           SELECT * 
             FROM ACCOUNT_T A, ORDER_T O
            WHERE A.BILLING_ID    = c_BILLING_DISC
...
BEGIN
    Pk01_Syslog.Write_msg('Start', c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- удаляем настройки печати документов
...
BEGIN
    ...
    -- заполняем информацию из профиля
    MERGE INTO AG_IMPORT_NPL_T PO
    USING (
    ...
    -- проставляем признак наличия групповой ссылки
