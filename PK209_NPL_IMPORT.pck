CREATE OR REPLACE PACKAGE PK209_NPL_IMPORT
IS
    --
    -- Пакет для создания клиентов бренда xTTK «Санкт-Петербургский ТЕЛЕПОРТ»
    --
    -- ==============================================================================
    c_PkgName   constant varchar2(30) := 'PK209_NPL_IMPORT';
    -- ==============================================================================
    ...
    c_BILLING_NPL    CONSTANT INTEGER := 2099; -- Тестовый биллинг NPL
...
END PK209_NPL_IMPORT;
/
CREATE OR REPLACE PACKAGE BODY PK209_NPL_IMPORT
IS
...
BEGIN
    ...
    UPDATE ACCOUNT_T A
     ...
     WHERE A.BILLING_ID = c_BILLING_NPL;
...
BEGIN
    ...
    SELECT * 
     ...
     WHERE CUSTNO IN (
           SELECT DISTINCT CUSTNO FROM AG_IMPORT_NPL_T K
       )
       AND NOT EXISTS (
    ...
    */
    ...
    DELETE FROM PINDB_NPL_ORDERS_T PO
     WHERE NOT EXISTS (
         SELECT * 
           FROM AG_IMPORT_NPL_T K 
          WHERE PO.CUSTNO = K.CUSTNO
     );
    v_count := SQL%ROWCOUNT;
...
--         AND P.CURRENCY_SECONDARY = 0
    ...
    LOOP
      ...
      Pk05_Account.Set_billing(
                   ...
                   p_billing_id => c_BILLING_NPL  -- -> Pk00_Const.c_BILLING_OLD
               );
...
BEGIN
    ...
    WITH PO AS (
        ...
        WHERE IP_USAGE_RATE_PLAN IS NULL
     ...
     WHERE EXISTS (
           SELECT * 
            ...
            WHERE PO.ORDER_NO    = O.ORDER_NO
              ...
              AND O.ACCOUNT_ID   = SA.ACCOUNT_ID
     ...
     WHERE EXISTS (
         SELECT * 
          ...
          WHERE AD.ACCOUNT_ID = A.ACCOUNT_ID
            AND A.BILLING_ID  = c_BILLING_NPL
     );
    v_count := SQL%ROWCOUNT;
