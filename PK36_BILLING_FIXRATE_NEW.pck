CREATE OR REPLACE PACKAGE PK36_BILLING_FIXRATE_NEW
IS
    -- ==============================================================================
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    PROCEDURE Lock_processing(p_task_id IN INTEGER);
    
END PK36_BILLING_FIXRATE_NEW;
/
CREATE OR REPLACE PACKAGE BODY PK36_BILLING_FIXRATE_NEW
IS
...
BEGIN
    RETURN p_rate_value * ROUND(p_date_to - p_date_from) / 30;
END;
...
PROCEDURE Lock_processing(p_task_id IN INTEGER)
IS
    v_prcName       CONSTANT VARCHAR2(30) := 'Lock_processing';
...
BEGIN
    ...
    FOR rl IN (
          ...
          FROM (        
            SELECT ROW_NUMBER() OVER (PARTITION BY L.ORDER_ID ORDER BY L.DATE_FROM) RN,
             ...
             WHERE L.ORDER_ID = I.ORDER_ID
               ...
               AND TRUNC(L.DATE_FROM+1) <= TRUNC(L.DATE_TO) -- блокировки меньше 1 суток исключаем
        ...
        IF v_u_item_id IS NOT NULL THEN
            ...
            v_u_count := v_i_count + SQL%ROWCOUNT;
        END IF;
