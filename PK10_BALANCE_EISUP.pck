...
IS
    ...
    -- ========================================================================= --
    -- ------------------------------------------------------------------------- --
    -- Просмотр договоров на которых один л/с. Считаем их простыми. 
    -- ------------------------------------------------------------------------- --
    ...
    -- ------------------------------------------------------------------------- --
    -- Просмотр оборотов на договоре за указанный период 
    -- ------------------------------------------------------------------------- --
    ...
    -- ------------------------------------------------------------------------- --
    -- Просмотр информации по счетам выгруженным в TPI за период
    -- ------------------------------------------------------------------------- --
...
BEGIN
    ...
    OPEN p_recordset FOR
      ...
      SELECT AP.ACCOUNT_ID,   -- id л/с (можно не отображать в GUI)
...
BEGIN
    ...
    OPEN p_recordset FOR
      ...
      CBA AS ( -- обороты лицевым счетам договора за указанный период - - --
      SELECT * FROM (
        ...
        WHERE RN = 1 
      ) -- обороты по договору за указанный период - - - - - - - - - - - - - --
      SELECT CBA.REP_PERIOD_ID,               -- id периода
           CBA.CONTRACT_ID,                 -- id договора
           SUM(CBA.IN_BALANCE)  IN_BALANCE, -- входящее сальдо (на первый день  месяца)
           SUM(CBA.TOTAL)       TOTAL,      -- принято счетов на сумму
           SUM(CBA.RECVD)       RECVD,      -- принято платежей на сумму
           SUM(CBA.OUT_BALANCE) OUT_BALANCE -- исходящее сальдо (на последнюю секунду последнего дня месяца)
      FROM CBA 
...
BEGIN
    ...
    OPEN p_recordset FOR
      ...
      -- Запросы для сверки платежей
      -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
      WITH AP AS (    -- профиль на конец указанного периода
        SELECT * FROM (
         ...
         WHERE RN = 1 
      ), P AS (   -- платежи BRM поступившие на л/с в указанном периоде
        SELECT AP.CONTRACT_ID, AP.CONTRACT_NO, AP.ACCOUNT_NO, P.*
          FROM AP, PAYMENT_T P
         WHERE AP.ACCOUNT_ID = P.ACCOUNT_ID
           AND P.REP_PERIOD_ID = p_period_id
      ), PS AS (  -- платежи BRM поступившие на договор в указанном периоде
        SELECT P.CONTRACT_ID, P.CONTRACT_NO, SUM(RECVD) RECVD 
          FROM P
         GROUP BY P.CONTRACT_ID, P.CONTRACT_NO
      ), PBE AS ( -- связка с платежами ЕИСУП
          SELECT PS.*, T.* 
            FROM PS FULL OUTER JOIN EISUP_RP_BALANCE_TMP_1 T ON (PS.CONTRACT_ID = T.BRM_CONTRACT_ID)
      ), PBE_OK AS (
          -- договоры по которым совпали суммы платежей 
          SELECT * FROM PBE
           WHERE CONTRACT_ID IS NOT NULL
             AND BRM_CONTRACT_ID IS NOT NULL
             AND RECVD = PAY_TOTAL    -- 291 - плохо
             AND RECVD != 0
      ), PBE_BAD_SUM AS (
          -- договоры по которым есть платежи и в BRM и в ЕИСУП, но суммы не совпадают
          SELECT * FROM PBE
           WHERE CONTRACT_ID IS NOT NULL
             AND BRM_CONTRACT_ID IS NOT NULL
             AND RECVD != PAY_TOTAL    -- 3.251 - плохо
      ), PBE_BRM_NOT_FOUND AS (
          -- договоры по которым есть оплаты в ЕИСУП, но нет в BRM 
          SELECT * FROM PBE
           WHERE CONTRACT_ID IS NULL
             AND BRM_CONTRACT_ID IS NOT NULL
             AND PAY_TOTAL != 0        -- 14.010
      ), PBE_EISUP_NOT_FOUND AS (
          -- договоры по которым есть оплаты в BRM, но нет в ЕИСУП 
          SELECT * FROM PBE
           WHERE CONTRACT_ID IS NOT NULL
             AND BRM_CONTRACT_ID IS NULL
             AND RECVD != 0             -- 6
      ), PBE_CHECK AS (
          -- проверяем соответствие PAYMENT_T и REP_PERIOD_INFO_T в BRM
          -- расхождения могут появиться при работе с платежами задним числом (пока не запрещено)
