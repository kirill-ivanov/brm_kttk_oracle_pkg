CREATE OR REPLACE PACKAGE PK1110_OPR_TARIFFING
IS

    gc_PkgName       CONSTANT varchar2(36) := 'PK1110_OPR_TARIFFING';
    ...
    */
    ...
    FUNCTION Trf_Oper_Table_Old(pr_Call     ref_Trf_Op,
...
END PK1110_OPR_TARIFFING;
/
CREATE OR REPLACE PACKAGE BODY PK1110_OPR_TARIFFING
IS
...
BEGIN
   ...
   -- Проверка есть ли закрытые периоды в заданном диапазоне дат
    IF pk114_items.Check_Rep_Period(p_Date_From, p_Date_To, NULL) = 0 AND p_Test_BDR_Table IS NULL
    THEN
        RETURN;
    END IF;     
...
-- Получаем отчетный период  
    ...
    l_SQL :=   
       ...
       '     SELECT NVL(:l_Rep_Period, x.bill_date) rep_period, ' ||
           ...
           '        :l_BDR_Source, x.trf_type, x.BDR_Status, ' || 
       ...
       '       FROM TABLE(CAST(PK1110_OPR_TARIFFING.Trf_Oper_Table( ' || CHR(10) ||
              '    CURSOR(SELECT /*+ PARALLEL(t 5) */ ' || CHR(10) ||
              '                  t.cdr_id, t.bdr_type, t.start_time, ' || CHR(10) ||
              '                  t.duration, t.Sw_Name, t.trunk_group_in, t.trunk_group_out, ' || CHR(10) ||
              '                  t.subs_a, t.subs_b, t.abn_a, t.called_number, ' || CHR(10) ||
              '                  t.order_id_a, t.gln_a, t.local_time_a, t.utc_offset_a, ' || CHR(10) ||
              '                  t.order_id_b, t.gln_b, t.local_time_b, t.utc_offset_b, ' || CHR(10) ||
              '                  t.row_id ' || CHR(10) ||
            ...
            --  '            ORDER BY 2 ASC ' ||
              '         ), :p_Data_Type, :p_Rep_Period_Id, :p_Task_Id) AS OP_BDR_COLL) ' || CHR(13) || 
              '       ) x ';
    ...
    EXECUTE IMMEDIATE l_SQL                 
      USING l_Rep_Period,  
            ...
            l_BDR_Type_Id,
            p_Data_Type, l_Rep_Period_Id, p_Task_Id;    
...
BEGIN
    ...
    IF p_Data_Type = 'SPB' THEN
        l_Def := gc_Zone_Def_SPb_Id ; -- id зоны с Питерскими DEF-кодами
    ELSIF p_Data_Type = 'NOVTK' THEN
        l_Def := gc_Zone_Def_Nvr_Id ; -- id зоны с DEF-кодами Новороссийска
    END IF;    
...
IS
    ...
    TYPE ret_col IS TABLE OF PIN.OP_BDR_TYPE INDEX BY PLS_INTEGER;
    t_ret_col ret_col;
    ...
    LOOP
       ...
       -- получаем id коммутатора
        SELECT switch_id
          INTO l_Sw_Id
          FROM switch_t  
         WHERE switch_code = l_cur.sw_name;          
        ...
        LOOP -- Перебор направлений. Сначала тарификация стороны А, а затем Б (если требуется)
           ...
           -- приводим телефонные номера к стандартному виду
            ret_rec.abn_a := pin.Norm_Ph_Number(p_Network_Code => l_Network_Code,
                                                p_PH_Num       => l_cur.Abn_A);       
           
            ret_rec.abn_b := pin.Norm_Ph_Number(p_Network_Code => l_Network_Code,
                                                p_PH_Num       => l_cur.Abn_B,
                                                p_Side         => 'B');       

           -- получаем имя направления
            ret_rec.pref_b := pin.Get_Direction(p_Ph_Number => ret_rec.abn_b,
            ...
            LOOP 
                ...
                THEN
                    ...
                    ret_rec.Order_Id := Pk120_Bind_Operators.Get_XTTK_Op_Order_Id
                                                   (p_Network_Code => l_Network_Code,
                                                    p_Start_Time   => ret_rec.Start_Time,
                                                    p_Sw_Name      => l_cur.Sw_Name,
                                                    p_Tg           => l_Tg, 
                                                    p_Ph_Number    => (CASE l_Side 
                                                                           WHEN 'A' THEN l_cur.abn_a
                                                                           WHEN 'B' THEN l_cur.called_number
                                                                       END),
                    ...
                    IF ret_rec.Order_Id < 0 THEN
                       -- запоминоаем оригинальный код ошибки, что бы у BDR-ов и CDR-ов не было расхождения
                ...
                BEGIN        
                    ...
                    SELECT os.order_swtg_id, os.op_sw_id, o.account_id, o.rateplan_id
                      INTO ret_rec.order_swtg_id, l_Op_Sw_Id, ret_rec.Account_Id, ret_rec.RatePlan_Id
                      FROM SWITCH_T s,
                           X07_ORDER_SWTG_T os,
                           ORDER_T o
                     WHERE s.switch_code = l_cur.sw_name
                       AND o.order_id = os.order_id
                       AND os.order_id   = ret_rec.Order_Id
                       AND os.switch_id  = s.switch_id
                       AND LOWER(os.trunkgroup) = LOWER(l_Tg)
                       AND (
                            (os.Trunkgroup_No = l_Order_Ph OR os.Trunkgroup_No IS NULL )
                             OR
                            p_Data_Type = 'NOVTK'
                           )  
                       AND ret_rec.Bill_Date BETWEEN os.date_from AND NVL(os.date_to, gc_MaxDate);
                ...
                IF ret_rec.Account_Id IS NOT NULL THEN
                   ...
                   --- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                   -- Перебираем все возможные вар-ты тарифов
                   -- +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                    ...
                    LOOP
                        ...
                        IF l_Side = 'A' AND l_Trf_Step = 1 THEN
                    
                            ret_rec.trf_type := pin.pk00_const.c_OP_RATE_PLAN_TYPE_RIP; -- 6
                        
                           -- является или нет входящий звонок завершением на нашей ИП
                            ret_rec.BDR_Status :=
                                 Get_XTTK_R_RP_ID_Old(p_OP_Sw_Id    => l_Op_Sw_Id,
                                                  p_Phone       => ret_rec.Abn_B,
                                                  p_Date        => ret_rec.Bill_Date,
                                                  p_RP_Type     => pin.pk00_const.c_OP_RATE_PLAN_TYPE_RIP,
                                                  p_RatePlan_Id => ret_rec.RatePlan_Id,                                       
                                                  p_Trf_Vol     => ret_rec.vol_type,       -- OUT
                                                  p_SubSrv_Id   => ret_rec.subservice_id,  -- OUT
                                                  p_Op_Rp_Id    => ret_rec.op_rate_plan_id -- OUT
                                                 ); 
                
                        ELSIF l_Side = 'A' AND l_Trf_Step = 2 THEN
                          
                            ret_rec.trf_type := pin.pk00_const.c_OP_RATE_PLAN_TYPE_DT; -- 1
                          
                            -- ищем доход завершение
                            ret_rec.BDR_Status := 
                                Get_XTTK_D_RP_ID_Old(p_Switch_Id   => l_Sw_Id,
                                                 p_Phone       => ret_rec.Abn_B,
                                                 p_Date        => ret_rec.Bill_Date,
                                                 p_RP_Type     => pin.pk00_const.c_OP_RATE_PLAN_TYPE_DT,
                                                 p_RatePlan_Id => ret_rec.RatePlan_Id, 
                                                 p_Trf_Vol     => ret_rec.vol_type,       -- OUT
                                                 p_SubSrv_Id   => ret_rec.subservice_id,  -- OUT,
                                                 p_Op_Rp_Id    => ret_rec.op_rate_plan_id -- OUT
                        ...
                        ELSIF l_Side = 'A' AND l_Trf_Step = 3 THEN

                            ret_rec.trf_type := pin.pk00_const.c_OP_RATE_PLAN_TYPE_RI; -- 4

                            -- ищем расход инициирование
                            IF (
                                UPPER(p_Data_Type) = 'SPB' AND 
                                (ret_rec.Abn_B LIKE '7812%' OR LENGTH(ret_rec.Abn_B) < 11)
                               ) 
                               OR 
                               (
                                UPPER(p_Data_Type) = 'NOVTK' AND 
                                (ret_rec.Abn_B LIKE '78617%' OR LENGTH(ret_rec.Abn_B) < 11)
                               )                               
                            THEN                    
                            
                               -- считаем соединение местным
                                ret_rec.BDR_Status :=
                                   Get_XTTK_R_RP_ID_Old(p_OP_Sw_Id    => l_Op_Sw_Id,
                                                    p_Phone       => ret_rec.Abn_A,
                                                    p_Date        => ret_rec.Bill_Date,
                                                    p_RP_Type     => pin.pk00_const.c_OP_RATE_PLAN_TYPE_RI,
                                                    p_RatePlan_Id => ret_rec.RatePlan_Id,                                       
                                                    p_Trf_Vol     => ret_rec.vol_type,       -- OUT
                                                    p_SubSrv_Id   => ret_rec.subservice_id,  -- OUT
                                                    p_Op_Rp_Id    => ret_rec.op_rate_plan_id -- OUT
                            ...
                            ELSE                   
                                ...
                                BEGIN
                                    ...
                                    l_Prefix := Get_Def(ret_rec.Abn_B, p_Data_Type);
                                   ...
                                   /*  SELECT s.srv_id
                                      INTO ret_rec.subservice_id --srv_id
                                      FROM x07_srv_dct s
                                     WHERE s.srv_key = 'ZINIT001'; */ --s.subservice_id = 1006; -- услуга зонового инициирования
                                ...
                                EXCEPTION
                                       ...
                                       -- Номер Б не DEF. Т.е. звонок МГ/МН
                                        BEGIN
                                            ...
                                            IF l_cur.tg_out = 'ROSTO' THEN -- ТГ Ростелекома
                                                 ...
                                                 /*SELECT s.srv_id
                                                  INTO ret_rec.subservice_id 
                                                  FROM x07_srv_dct s
                                                 WHERE s.srv_key = 'IINIT003';*/ -- инициирование на МНМГ Ростелекома
                                            ...
                                            ELSIF l_cur.tg_out IN ('KTTKO','КТТК40') THEN -- ТГ ТТК
                                                ...
                                                /* SELECT s.srv_id
                                                  INTO ret_rec.subservice_id 
                                                  FROM x07_srv_dct s
                                                 WHERE s.srv_key = 'IINIT002';*/ -- инициирование на МНМГ ТрансТелекома                                                
                                            ...
                                            ELSE
                                                ...
                                                /* SELECT s.srv_id
                                                  INTO ret_rec.subservice_id 
                                                  FROM x07_srv_dct s
                                                 WHERE s.srv_key = 'IINIT001';*/ -- инициирование на МНМГ                                                                            
                                   ...
                                   -- WHEN too_many_rows THEN
                                    --    NULL; -- номер Питерского DEF. Ничего не делаем, останется ошибка поиска.
                                END;     
                            
                                IF ret_rec.BDR_Status = pin.pk00_const.c_Ret_OK THEN -- еслу услуга определена
                            
                                    -- ищем тар. план для новой услуги 
                                    ret_rec.BDR_Status := Get_XTTK_RP_ID(p_RP_Type     => pin.pk00_const.c_OP_RATE_PLAN_TYPE_RI,
                                                                         p_RatePlan_Id => ret_rec.RatePlan_Id,
                                                                         p_Trf_Vol     => ret_rec.vol_type,       -- OUT
                                                                         p_Op_Rp_Id    => ret_rec.op_rate_plan_id -- OUT
                                                                        );
                        ...
                        ELSIF l_Side = 'B' AND l_Trf_Step = 1 THEN

                            ret_rec.trf_type := pin.pk00_const.c_OP_RATE_PLAN_TYPE_DIP; -- 5
                            ...
                            ret_rec.BDR_Status := 
                                Get_XTTK_D_RP_ID_Old(p_Switch_Id   => l_Sw_Id,
                                                 p_Phone       => ret_rec.Abn_B,
                                                 p_Date        => ret_rec.Bill_Date,
                                                 p_RP_Type     => pin.pk00_const.c_OP_RATE_PLAN_TYPE_DIP,
                                                 p_RatePlan_Id => ret_rec.RatePlan_Id, 
                                                 p_Trf_Vol     => ret_rec.vol_type,       -- OUT
                                                 p_SubSrv_Id   => ret_rec.subservice_id,  -- OUT
                                                 p_Op_Rp_Id    => ret_rec.op_rate_plan_id -- OUT
                        ...
                        ELSIF l_Side = 'B' AND l_Trf_Step = 2 THEN
                            ...
                            ret_rec.BDR_Status := 
                                  Get_XTTK_D_RP_ID_Old(p_Switch_Id   => l_Sw_Id,
                                                   p_Phone       => ret_rec.Abn_A,
                                                   p_Date        => ret_rec.Bill_Date,
                                                   p_RP_Type     => pin.pk00_const.c_OP_RATE_PLAN_TYPE_DI,
                                                   p_RatePlan_Id => ret_rec.RatePlan_Id,
                                                   p_Trf_Vol     => ret_rec.vol_type,       -- OUT
                                                   p_SubSrv_Id   => ret_rec.subservice_id,  -- OUT
                                                   p_Op_Rp_Id    => ret_rec.op_rate_plan_id -- OUT                                     
                            ...
                            IF ret_rec.BDR_Status != 0 AND 
                               LENGTH(ret_rec.Abn_B) >= 11 AND   
                               (
                                (UPPER(p_Data_Type) = 'SPB' AND ret_rec.Abn_B NOT LIKE '7812%')
                                 OR
                                (UPPER(p_Data_Type) = 'NOVTK' AND ret_rec.Abn_B NOT LIKE '78617%') 
                               )  
                            THEN
                                -- Ошибка поиска тарифа и звонок не местный. 
                                ...
                                BEGIN
                                    ...
                                    IF l_Prefix IS NULL THEN -- не DEF
                                        RAISE no_data_found;
                                    END IF;
                                ...
                                IF ret_rec.BDR_Status = pin.pk00_const.c_Ret_OK THEN -- еслу услуга определена
                                    ...
                                    -- ищем тар. план для новой услуги 
                                    ret_rec.BDR_Status := Get_XTTK_RP_ID(p_RP_Type     => pin.pk00_const.c_OP_RATE_PLAN_TYPE_DI,
                                                                         p_RatePlan_Id => ret_rec.RatePlan_Id,
                                                                         p_Trf_Vol     => ret_rec.vol_type,       -- OUT
                                                                         p_Op_Rp_Id    => ret_rec.op_rate_plan_id -- OUT
                                                                        );
                        ...
                        ELSIF l_Side = 'B' AND l_Trf_Step = 3 THEN
                            ...
                            ret_rec.BDR_Status :=
                                Get_XTTK_R_RP_ID_Old(p_OP_Sw_Id    => l_Op_Sw_Id,
                                                 p_Phone       => ret_rec.Abn_B,
                                                 p_Date        => ret_rec.Bill_Date,
                                                 p_RP_Type     => pin.pk00_const.c_OP_RATE_PLAN_TYPE_RT,
                                                 p_RatePlan_Id => ret_rec.RatePlan_Id,
                                                 p_Trf_Vol     => ret_rec.vol_type,       -- OUT
                                                 p_SubSrv_Id   => ret_rec.subservice_id,  -- OUT
                                                 p_Op_Rp_Id    => ret_rec.op_rate_plan_id -- OUT
                        ...
                        LOOP -- цикл поиска цены по услуге. Если услуга назначена по исход. ТГ и прайс
                             -- найден не будет, то будем искать по родительской услуге
                           ...
                           -- получаем serviсe_id и subservice_id по ЦБ 
                            IF ret_rec.BDR_Status = pin.pk00_const.c_RET_OK THEN
                                ...
                                BEGIN
                                    ...
                                    SELECT s.subservice_id, ss.service_id
                                      INTO ret_rec.parent_subsrv_id, ret_rec.service_id
                                      FROM x07_srv_dct s,
                                           service_subservice_t ss
                                     WHERE s.srv_id = ret_rec.subservice_id
                                       AND s.subservice_id = ss.subservice_id(+); 
                            ...
                            IF ret_rec.BDR_Status = pin.pk00_const.c_RET_OK THEN
                                ...
                                -- Получаем тарифицруемое время   
                                ret_rec.bill_minutes := PK1110_OPR_TARIFFING.Get_Bill_Duration(p_Seconds  => ret_rec.Duration,
                            ...
                            IF l_TG_Tariff = TRUE THEN
                                ...
                                BEGIN
                                    ...
                                    /*SELECT subservice_id
                                      INTO ret_rec.subservice_id --srv_id
                                      FROM subservice_t
                                     WHERE subservice_key = 'IINIT001';*/
                                     
                                     SELECT s.srv_id, s.subservice_id, ss.service_id
                                       INTO ret_rec.subservice_id, ret_rec.parent_subsrv_id, ret_rec.service_id 
                                       FROM x07_srv_dct s,
                                            service_subservice_t ss
                                      WHERE s.srv_key = 'IINIT001' -- инициирование на МНМГ
                                        AND s.subservice_id = ss.subservice_id(+);                                   
                        ...
                        THEN                                             
                          ...
                          --  PIPE ROW (ret_rec); -- возвращаем протарифицированную запись
                            ...
                            t_ret_col(l_Trf_Step) := ret_rec; -- запоминаем протарифицированную запись
                            
                            IF ret_rec.bdr_status IN (pin.pk00_const.c_VOL_PRICE_NOT_FOUND,
                                                      pin.pk00_const.c_RET_OK) THEN
            ...
            -- возвращаем протарифицированную запись
            l_Trf_Step := t_ret_col.FIRST();

            WHILE l_Trf_Step IS NOT NULL 
            LOOP
              PIPE ROW (t_ret_col(l_Trf_Step));  
              l_Trf_Step := t_ret_col.NEXT(l_Trf_Step);
            END LOOP;        
...
IS
   ...
   -- инициализация частей тарифов, которые надо обрабатывать при тарификации 
    PROCEDURE Init_Trf_Types(p_Side varchar2)
    IS
                       ...
                       -- ELSIF l_Side = 'A' AND l_Trf_Step = 3 THEN
                        ...
                        THEN
                            ...
                            THEN
                                ...
                                ELSE -- звонок на DEF
                                   -- ++++ Ветка исключена 03.07.2015 по согласованию с Гуровым А.Ю. как неимеющая смысла +++++
