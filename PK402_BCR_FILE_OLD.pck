CREATE OR REPLACE PACKAGE PK402_BCR_FILE_OLD
IS
    --
...
END PK402_BCR_FILE_OLD;
/
CREATE OR REPLACE PACKAGE BODY PK402_BCR_FILE_OLD
IS
...
BEGIN    
    ...
    INSERT INTO INFRANET.BRM_ACCOUNT_T (
        ...
        BILLING_ID, SALES_MANAGER, DLV_METHOD 
    )
    WITH 
            ...
            FROM (
              SELECT ACCOUNT_ID, PROFILE_ID,
                     ROW_NUMBER() OVER (PARTITION BY ACCOUNT_ID 
                                        ...
                                        NVL(DATE_TO, TO_DATE('01.01.2050','dd.mm.yyyy')) DESC
                                       ) RN
      ...
      AP_CSS AS (
          SELECT AP.ACCOUNT_ID, AP.CONTRACTOR_ID, AP.BRANCH_ID, AP.AGENT_ID, C.CONTRACTOR AGENT
            FROM ACCOUNT_PROFILE_T AP, CONTRACTOR_T C
           WHERE AP.BRANCH_ID = 200
             AND AP.AGENT_ID = C.CONTRACTOR_ID
      ),
      D_SEG AS (
      ...
      ACC AS (
          SELECT DISTINCT 
                 ...
                 DECODE(A.ACCOUNT_TYPE, 
                        ...
                        'J', CM_MAX.COMPANY_NAME , NULL) CUST_NAME,
                 C.CLIENT_ID, 
                 C.DATE_FROM, 
                 C.DATE_TO, 
                 D_SEG.MSEG_ID, 
           ...
           WHERE A.ACCOUNT_ID        = AP_MAX.ACCOUNT_ID
             ...
             AND AP.ACCOUNT_ID       = AP_CSS.ACCOUNT_ID(+)
             --AND AP.BRAND_ID       = BR.BRAND_ID(+)
             ...
             AND C.CONTRACT_ID       = CM_MAX.CONTRACT_ID
