...
IS
    ...
    -- создать стандартный новый номер лицевого счета по правилам INFRANET
    FUNCTION New_std_account_no(
                       p_account_type IN CHAR     -- 'J'/'P'
                   ) RETURN VARCHAR2;
    
    -- создать стандартный новый номер лицевого счета по правилам INFRANET
    -- для "Регионального Продукта"
    FUNCTION New_rp_account_no RETURN VARCHAR2;
    ...
    -- Для ЦСС - новые клиенты заводятся в биллинг 2003
    -- создать новый лицевой счет, возвращает значения
    --   - положительное - ID лицевого счета клиента, 
    --   - при ошибке выставляет исключение
    FUNCTION New_account_css(
                   p_account_no    IN VARCHAR2,
                   p_account_type  IN VARCHAR2,
                   p_currency_id   IN INTEGER,
                   p_status        IN VARCHAR2,
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Проверить профили л/с на пересечение дат
    --
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    -- Добавить на л/с признак разрешение/запрет (Y/N) компенсации простоев
    -- - при ошибке выставляет исключение
    PROCEDURE Set_idl_enable (
                   p_account_id IN INTEGER,
                   p_idl_enable IN CHAR DEFAULT 'Y'
               );
...
IS
    ...
    v_count_rp   INTEGER;
BEGIN
    SELECT COUNT(*) INTO v_count_rp
        FROM contractor_t
       WHERE contractor LIKE '%(РП%';
        
    v_account_no := SUBSTR(p_contract_no, 1, 8);
    -- проверяем соответствие номера л/с шаблону CCnnnnnn
    IF LTRIM(SUBSTR(v_account_no,3),'1234567890') IS NOT NULL OR
        LTRIM(UPPER(SUBSTR(v_account_no,1,2)),'ABCDEFGHIJKLMNOPQRSTUVWXYZ') IS NOT NULL OR
        LENGTH(v_account_no) != 8 
    THEN
        -- переприсваиваем л/с стандартный номер
         IF v_count_rp = 1 THEN
              v_account_no := New_rp_account_no;
         ELSE   
            v_account_no := New_std_account_no('J');
         END IF;
    ELSE
...
IS
BEGIN
    RETURN 'X'||p_account_type||LPAD(SQ_ACCOUNT_NO.NEXTVAL,6,'0');
END;
...
IS
BEGIN
    RETURN 'RP'||LPAD(SQ_ACCOUNT_NO.NEXTVAL,6,'0');
END;
...
IS
    v_prcName     CONSTANT VARCHAR2(30) := 'New_account_css';
    v_account_id  INTEGER;
    v_billing_id  INTEGER;
BEGIN
    -- все клиенты ЦСС ведутся в биллинге Микротест, т.к. только там учитываются платежи
    v_billing_id := Pk00_Const.c_BILLING_MMTS;
...
IS
    ...
    v_count             INTEGER;
BEGIN
    -- добавляем проверку реквизитов в зависимости от биллинга 
    SELECT BILLING_ID, ACCOUNT_NO 
      INTO v_billing_id, v_account_no
      FROM ACCOUNT_T A
     WHERE A.ACCOUNT_ID = p_account_id;
    -- 
    -- проверяем, что это первый профиль
    SELECT COUNT(*) INTO v_count
      FROM ACCOUNT_PROFILE_T AP
     WHERE AP.ACCOUNT_ID = p_account_id;
    --
    -- получаем предопределенные id продавца и биллинга
    SELECT T.SELLER_ID, T.BILLINGS -- если 2006,2008 -> ACCOUNT_T.BILLING_ID ? 
      INTO v_seller_id, v_billings
      FROM BRANCH_SELLER_TMP T
     WHERE BRANCH_ID  = p_branch_id;
    --
    -- проверяем правильность установки account_t.billing_id
    IF v_billings in ('2006','2008') THEN
      IF v_billings != v_billing_id THEN
         v_billing_id := v_billings;
         IF v_count = 0 THEN  
            -- на первый раз можно поменять
            UPDATE ACCOUNT_T SET BILLING_ID = v_billing_id
             WHERE ACCOUNT_ID = p_account_id;
         ELSE
           -- между биллингами счет гулять не должен
           Pk01_Syslog.Raise_user_exception('account_id='||p_account_id||
                      'billing_id=' || v_billing_id || ' - не соответствет branch_id='||p_branch_id
                     , c_PkgName||'.'||v_prcName );
         END IF;
      END IF;
    END IF;
    -- для (РП) принудительно устанавливаем банк продавца
    IF v_billing_id IN (2006,2008) THEN
        --SELECT CB.BANK_ID INTO v_bank_id
        --  FROM CONTRACTOR_BANK_T CB
        -- WHERE CB.CONTRACTOR_ID = v_seller_id
        --   AND ROWNUM = 1;
        v_contractor_id := v_seller_id;
    ELSE 
        -- заглушка на 2001 биллинг
        --IF (v_billing_id = 2001) AND (p_contractor_id = 1) THEN 
        --    v_bank_id := 4;
        --ELSE
        --    v_bank_id := p_contractor_bank_id;
        --END IF;
    ...
    IF v_count = 0 THEN
        INSERT INTO COMPANY_T (CONTRACT_ID, COMPANY_NAME, SHORT_NAME, DATE_FROM, DATE_TO, COMPANY_ID, ACTUAL)
        SELECT p_contract_id, CS.CUSTOMER, CS.SHORT_NAME, p_date_from, NULL, SQ_CLIENT_ID.NEXTVAL, 'Y'
          FROM CUSTOMER_T CS
    ...
    ELSE
      -- проверяем на соответствие номера л/c номеру договора
      SELECT C.CONTRACT_NO INTO v_contract_no
        FROM CONTRACT_T C
       WHERE C.CONTRACT_ID = p_contract_id;
      -- проверяем корректность номера л/с, формат: XXNNNNNN
      IF v_account_no != v_contract_no THEN
         IF LTRIM(SUBSTR(v_account_no,3),'1234567890') IS NOT NULL OR
            LTRIM(UPPER(SUBSTR(v_account_no,1,2)),'ABCDEFGHIJKLMNOPQRSTUVWXYZ') IS NOT NULL OR
            LENGTH(v_account_no) != 8 
         THEN
            ...
            ELSE
              v_account_no := New_std_account_no('J');
            END IF;
            -- устанавливаем номер л/с
            UPDATE ACCOUNT_T SET ACCOUNT_NO = v_account_no
             WHERE ACCOUNT_ID = p_account_id;
         END IF;
      END IF;
    END IF;
    --
    IF v_billing_id = Pk00_Const.c_BILLING_MMTS AND v_bank_id NOT IN (1,2) THEN
        Pk01_Syslog.Raise_user_exception('account_id='||p_account_id||
                      'bank_id='||v_bank_id|| 'bill_id=' || v_billing_id || ' - неверно указаны реквизиты'
                     , c_PkgName||'.'||v_prcName );
    ELSIF v_billing_id != Pk00_Const.c_BILLING_MMTS AND v_bank_id IN (1,2) THEN
        Pk01_Syslog.Raise_user_exception('account_id='||p_account_id||
                      'bank_id='||v_bank_id|| 'bill_id=' || v_billing_id||' - неверно указаны реквизиты'
                     , c_PkgName||'.'||v_prcName );
    END IF;
    ...
    VALUES
       (Pk02_Poid.Next_account_profile_id, 
        ...
        v_contractor_id, p_branch_id, p_agent_id, 
        v_bank_id, p_vat, p_brand_id, p_date_from, p_date_to)
    RETURNING PROFILE_ID INTO v_profile_id;
    
    -- Если договор агентский (CONTRACT_TYPE_ID = 7107, тогда обновляем признак лицевого счета
    SELECT CONTRACT_TYPE_ID INTO v_contract_type_id
        FROM contract_t 
       WHERE contract_id = p_contract_id;
       
    IF v_contract_type_id = 7107 THEN
       UPDATE account_t
          SET STATUS = 'A'
       WHERE ACCOUNT_ID = p_account_id;       
    END IF;
...
IS
    v_prcName    CONSTANT VARCHAR2(30) := 'Check_profiles';
    v_retcode    INTEGER;
BEGIN
    -- построить курсор
    OPEN p_recordset FOR
      SELECT AP.ACCOUNT_ID, AP.PROFILE_ID, AP.DATE_FROM, AP.DATE_TO,
             MIN(B.BILL_DATE) MIN_BILL_DATE, MAX(B.BILL_DATE) MAX_BILL_DATE  
       ...
       GROUP BY AP.ACCOUNT_ID, AP.PROFILE_ID, AP.DATE_FROM, AP.DATE_TO
       ORDER BY AP.ACCOUNT_ID, AP.DATE_FROM
      ;
EXCEPTION
    WHEN OTHERS THEN
        v_retcode := Pk01_SysLog.Fn_write_Error('ERROR', c_PkgName||'.'||v_prcName);
...
IS
    v_prcName    CONSTANT VARCHAR2(30) := 'Set_idl_enable';
BEGIN
    UPDATE ACCOUNT_T A 
       SET A.IDL_ENB = p_idl_enable
     WHERE ACCOUNT_ID = p_account_id;
EXCEPTION   -- при ошибке выставляем исключение
    WHEN OTHERS THEN
        Pk01_Syslog.raise_Exception('ERROR', c_PkgName||'.'||v_prcName );
