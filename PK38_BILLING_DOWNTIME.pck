...
IS
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Привязка к заказам, л/с, позициям выставленных счетов 
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Сохранить отчет в схеме INFRANET 
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    ...
    -- Дублирование данных о простоях, данные о привязке сохраняются, о начислениях стираются
    -- это нужно для персчтетов простоев в дебетовых/корректировочных счетах 
    PROCEDURE Dup_downtimes( 
                 p_src_task_id IN INTEGER,  -- помечаем строки для дублирования задачей
                 p_dst_task_id IN INTEGER   -- получаем номер очередной задачи из последовательности
              );
...
BEGIN
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- 2) Привязка к заказам, л/с, позициям выставленных счетов 
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --    
    -- 4. Сохранить отчет в схеме INFRANET 
...
BEGIN
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- договор с РЖД л/с MS007316  (account_id = 2447046)
    -- в формуле расчета простоев нужно применять 1/360, а не 1/720 (стандартную) 
    -- по договору двойная компенсация
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
...
BEGIN
    ...
    UPDATE BRM_DOWNTIME_T D 
       SET IDL_NOTES = '(Простои '||
               ...
               WHEN D.RATE_RULE_ID = 2421 THEN TO_CHAR(CEIL((D.MINUTES - D.DOWNTIME_FREE)/60))  -- округляем в большую сторону до часа
               END||' час.)',
           IDL_VALUE = -1 * (
               CASE
               ...
               WHEN D.RATE_RULE_ID = 2432 THEN REC_OB_RATE_VALUE * ROUND((D.MINUTES - D.DOWNTIME_FREE)/60) / 320  -- Для РЖД, л/с MS007316 - по договору двойная компенсация
               END
               ),
           D.STATUS      = 4,
           D.STATUS_DATE = SYSDATE
    ...
    -- ошибки нулевая компенсация 
    UPDATE BRM_DOWNTIME_T D 
       SET D.STATUS = 0, 
           D.NOTES  = 'Величина компенсации 0',
           D.STATUS_DATE = SYSDATE
     WHERE D.TASK_ID = p_task_id
       AND D.STATUS  = 4
       AND D.IDL_VALUE = 0
    ...
    USING (
        SELECT D.DOWNTIME_ID, 
          ...
          FROM SLA_PERCENT_T SP, (
            SELECT D.DOWNTIME_ID, SLA_RATEPLAN_ID, '('||ROUND(MINUTES/60)||' час.)' IDL_NOTES, 
                   CASE
    ...
    SELECT DISTINCT 
     ...
     WHERE B.REP_PERIOD_ID = D.PERIOD_ID
       ...
       AND (
           ...
           )-- 25.11.2016 С.Макеев, по письму Н.Фоменко: "Простои на учитываются для закрытых на момент формирования простоя счетов"
       ;
    v_count := SQL%ROWCOUNT;
...
BEGIN
    -- выравниваем валюту в компонентах услуг REC/MIN -> IDL/SLA
    ...
    UPDATE BRM_DOWNTIME_T D
       SET D.STATUS      = -v_status,
           D.STATUS_DATE = SYSDATE,
           D.NOTES       = 'ошибка создания item-а'
     WHERE D.TASK_ID     = p_task_id
       AND D.STATUS      = v_status
       AND NOT EXISTS (
...
BEGIN
    ...
    -- поля отчета в исходной таблице
...
-- --------------------------------------------------------------------- --
-- Расформирование компенсаций за простои
-- ВНИМАНИЕ - на то что счет READY/CLOSED внимания не обращаю, 
-- этого требует процедура Recharge_downtimes
--
PROCEDURE Rollback_downtimes( p_task_id IN INTEGER )
IS
...
BEGIN
    ...
    DELETE FROM ITEM_T I
     WHERE I.CHARGE_TYPE IN (Pk00_Const.c_CHARGE_TYPE_IDL, 
       ...
       --AND I.ITEM_STATUS   = Pk00_Const.c_ITEM_STATE_OPEN   -- все-таки должен быть открыт
    ...
    UPDATE BRM_DOWNTIME_T D
     ...
     WHERE D.TASK_ID = p_task_id
       AND D.STATUS > 3      -- статусы, где были ошибки раньше сохраняем
       AND NOT EXISTS (
           SELECT * FROM ITEM_T I
