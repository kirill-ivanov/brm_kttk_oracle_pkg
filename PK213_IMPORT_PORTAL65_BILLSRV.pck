CREATE OR REPLACE PACKAGE PK213_IMPORT_PORTAL65_BILLSRV
IS
    --
    -- Пакет для создания клиентов бренда xTTK «Санкт-Петербургский ТЕЛЕПОРТ»
    --
    -- ==============================================================================
    c_PkgName   constant varchar2(30) := 'PK213_IMPORT_PORTAL65_BILLSRV';
    -- ==============================================================================
    ...
    c_BILLING_ID          CONSTANT INTEGER := 2009; -- для тестирования
...
--    c_LOAD_CODE_OK        CONSTANT INTEGER := 2;    -- ОК
    ...
    -- Импорт заказов по списку с ослабленными фильтрами, используем для расходных заказов
...
END PK213_IMPORT_PORTAL65_BILLSRV;
/
CREATE OR REPLACE PACKAGE BODY PK213_IMPORT_PORTAL65_BILLSRV
IS
...
-- ------------------------------------------------------------------- --
-- Импорт заказов по списку с ослабленными фильтрами
-- используем для расходных заказов
-- ------------------------------------------------------------------- --
PROCEDURE Import_orders_by_list
IS
...
BEGIN
    ...
    USING (
        SELECT RID
          FROM (
            SELECT ROW_NUMBER() OVER (PARTITION BY XO.BRM_ORDER_NO ORDER BY ORDER_DATE DESC) RN,
                   ROWID RID, 
                   XO.* 
    ...
    UPDATE PK213_PINDB_ORDERS_T XO
       SET LOAD_CODE = -1, 
           LOAD_STATUS = 'Ошибка, заказ уже существует в BRM'
     WHERE EXISTS (
    ...
    -- проставляем order_id
    UPDATE PK213_PINDB_ORDERS_T XO 
       SET XO.BRM_ORDER_NO = SUBSTR(XO.ORDER_NO,1,100), 
...
BEGIN
    ...
    UPDATE PK213_PINDB_ORDERS_T XO
       SET BRM_CHARGE_TYPE = 'USG',
           BRM_SUBSERVICE_ID = 
             CASE 
                WHEN XO.BRM_SERVICE_ID = 142 THEN NULL-- 'Динамически, из названия'
                WHEN XO.BRM_SERVICE_ID = 125 THEN 5 -- 'Только местный трафик'
                WHEN XO.BRM_SERVICE_ID IN (140, 167) THEN 6 -- 'Только зоновый трафик'
                ELSE 1 -- 'МГ-МН' 2 - подставим при формировании order_body
             END
      ...
      BEGIN
        ...
        IF v_count = 0 THEN
            ...
            INSERT INTO ORDER_T (
               ...
               c_RATEPLAN_BILSRV_ID, -- проставляем обязательно, как признак внешнего обслуживания
        ...
        ELSIF r_ord.brm_subservice_id = 5 OR   -- местное соединение
              r_ord.brm_subservice_id = 6 THEN -- внутризоновое присоединение
            --
            INSERT INTO ORDER_BODY_T(
                ORDER_BODY_ID, ORDER_ID, SUBSERVICE_ID, CHARGE_TYPE, 
                DATE_FROM, DATE_TO, 
