CREATE OR REPLACE PACKAGE Pk1110_Opr_ReTrf_New AS

    gc_PkgName    CONSTANT varchar2(30) := 'Pk1110_Opr_ReTrf_New'; 
...
END Pk1110_Opr_ReTrf_New;
/
CREATE OR REPLACE PACKAGE BODY Pk1110_Opr_ReTrf_New AS
    ...
    IF p_Task_Id = gc_SRC_AUDIT AND UPPER(p_Data_Type) IN ('SPB', 'NOVTK') THEN
        ...
        IF UPPER(p_Data_Type) = 'SPB' AND p_Side = 'A' THEN
           -- Определяем данные по номерам SIP, подлежащие пересчету 
            ...
            SELECT Pk120_Bind_Operators_New.gc_SIP_SW, Pk120_Bind_Operators_New.gc_SIP_TG, NULL, 
                   TO_DATE(a.add_value,'dd.mm.yyyy hh24:mi:ss'), gc_RSX50_TG, p_Data_Type,
                   Pk120_Bind_Operators_New.gc_SIP_Main_Ph, a.num_value caller_number, 
        ...
        IF UPPER(p_Data_Type) = 'SPB' AND p_Side = 'A' THEN
           ...
           -- Загружаем все номера номера SIP, которые могут быть у данного л/счета 
            INSERT INTO TMP01_REBIND_CDR (
                   ...
                   ERR_CODE, SERVICE_ID,
                   TRF_SIDE)
            SELECT Pk120_Bind_Operators_New.gc_SIP_SW, NVL(q.tg, Pk120_Bind_Operators_New.gc_SIP_TG), NULL, 
              ...
              FROM TABLE(CAST(mdv.pck_tools.DistribRange(CURSOR(
                                             SELECT r.phone_from, r.phone_to, TO_CHAR(r.order_id)
                                              ...
                                              WHERE q.task_id = p_Task_Id 
                                                ...
                                                AND (q.tg = Pk120_Bind_Operators_New.gc_SIP_TG OR q.tg IS NULL)
...
--RAISE no_data_found;                 
        ...
        IF --p_Test_Tbl IS NULL AND 
           ...
           -- обновляем данные о л/с в таблице CDR-ов
            l_Update := Pk120_Bind_Operators_New.Update_CDR_Op_Id(--p_Data_Type    => p_Data_Type,
                                                                  p_Data_Table   => l_Tmp_Table,
                                                                  p_Date_From    => l_Date_From,
                                                                  p_Date_To      => l_Date_To,
                                                                  p_Id_Log       => 0
        ...
        THEN
            ...
            pk1110_opr_tariffing_new.Recalc_Op_V_Tariff(p_Data_Type   => p_Data_Type,
