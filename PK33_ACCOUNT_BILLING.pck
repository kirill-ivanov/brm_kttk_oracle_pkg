CREATE OR REPLACE PACKAGE PK33_ACCOUNT_BILLING
IS
    --
    ...
    --
    -- ==============================================================================
    c_PkgName   constant varchar2(30) := 'PK33_ACCOUNT_BILLING';
    -- ==============================================================================
...
END PK33_ACCOUNT_BILLING;
/
CREATE OR REPLACE PACKAGE BODY PK33_ACCOUNT_BILLING
IS
...
BEGIN
    ...
    --
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- переводим позиции начислений и корректировок счета в состояние READY,
    -- заполняем поля налогов позиций начислений и корректировок для системы отчетов
    UPDATE ITEM_T I 
       SET (I.REP_GROSS, I.REP_TAX, I.ITEM_STATUS) = (
            SELECT 
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- если пустой проставляем статус и выходим
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    IF v_count = 0 THEN
      UPDATE BILL_T B
         SET B.BILL_STATUS = Pk00_Const.c_BILL_STATE_EMPTY, 
             B.TOTAL = 0, 
             B.GROSS = 0, 
             B.TAX   = 0, 
             B.DUE   = 0, 
      ...
      Pk01_Syslog.Write_msg('Stop, bill is empty', c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );
      
    ELSE
      -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
      -- формируем инвойсы для указанного счета
      --
...
BEGIN
    ...
    WITH MONTH_PERIOD AS ( 
        SELECT REP_PERIOD_ID, ACCOUNT_ID,
         ...
         FROM (
            -- получаем полную задолженность по периодам, по выставленным счетам
            SELECT B.REP_PERIOD_ID, B.ACCOUNT_ID,  
                   (B.GROSS+B.TAX) BILL_TOTAL, B.GROSS,  
                   0 RECVD, 0 ADVANCE 
              FROM BILL_T B
