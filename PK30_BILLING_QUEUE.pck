...
IS
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Проверить на допустимость периодов счетов, стоящих в очереди
    -- В очереди не должно быть кредит-нот и счетов из закрытых периодов
...
BEGIN
    ...
    USING (
        SELECT Q.BILL_ID,
         ...
         WHERE Q.TASK_ID       = p_task_id
           ...
           AND A.BILLING_ID   != Pk00_Const.c_BILLING_RP -- 2008 - в тестовом можно все
           AND (
    ...
    v_count := SQL%ROWCOUNT;
    -- фиксируем ошибки в системе логирования
    IF v_count > 0 THEN
      -- кол-во записей в очереди, если одна, то выставим исключение
      SELECT COUNT(*) 
        INTO v_qcount
      ...
      -- удаляем ошибочные записи из очереди
      DELETE FROM BILLING_QUEUE_T Q 
       WHERE Q.TASK_ID = p_task_id
      ...
      -- если очередь пустая выставляем исключение
...
BEGIN
    ...
    -- Считаютя криво!!! используем полный перерасчет в конце закрытия
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
