...
DECLARE
    ...
    v_flag         BOOLEAN;
BEGIN
    -- проставляем дату изменения
    :NEW.MODIFY_DATE := SYSDATE;
    v_flag := false;
    -- проставляем правила тарификации если они не заданы
    IF :NEW.RATE_RULE_ID IS NULL THEN
      IF :NEW.CHARGE_TYPE = 'REC' THEN
        :NEW.RATE_RULE_ID := 2402;  -- c_RATE_RULE_ABP_STD -- Стандартное начисление абонплаты
        v_flag := true;
      ELSIF :NEW.CHARGE_TYPE = 'MIN' THEN
        :NEW.RATE_RULE_ID := 2401;  -- c_RATE_RULE_MIN_STD -- Стандартное начисление до минимальной суммы
        v_flag := true;
      ELSIF :NEW.CHARGE_TYPE = 'IDL' THEN
        :NEW.RATE_RULE_ID := 2404;  -- c_RATE_RULE_IDL_STD -- Компенсация простоев, стандартная схема
        v_flag := true;  
      END IF;
      -- сохраняем отчет
      IF v_flag = true THEN
      ...
      END IF;
    END IF;
    -- проверяем наличие признака TAX_INCL (включения налогов в тариф)
    IF :NEW.TAX_INCL IS NULL AND :NEW.CHARGE_TYPE IN ('REC','MIN','IDL') THEN
        -- проставляем по стандартному правилу 'J' - 'N', 'P' - 'Y'
