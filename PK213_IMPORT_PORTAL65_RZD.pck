CREATE OR REPLACE PACKAGE PK213_IMPORT_PORTAL65_RZD
IS
    --
    -- Пакет для создания клиентов бренда xTTK «Санкт-Петербургский ТЕЛЕПОРТ»
    --
    -- ==============================================================================
    c_PkgName   constant varchar2(30) := 'PK213_IMPORT_PORTAL65_RZD';
    -- ==============================================================================
    ...
    c_BILLING_ID          CONSTANT INTEGER := 2002; -- для тестирования
...
--    c_LOAD_CODE_OK        CONSTANT INTEGER := 2;    -- ОК
    ...
    с_RATESYSTEM_ID       CONSTANT INTEGER := 1209; -- тарификация в billngServer В.Малиновского
    ...
    -- ----------------------------------------------------------------------------- --
    -- Загрузка информации по заказам, по данным В.Малиновского
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Установить ID услуги (устанавливаем при загрузке)
    ...
    -- проверка правильности формирования компонентов услуг
    ...
    CURSOR c_ORD IS (
       SELECT 
        ...
        WHERE 1=1
          AND LOAD_CODE = 0
          --AND LOAD_STATUS LIKE 'ORDER_BODY_T IS EMPTY%'
    )FOR UPDATE;
    
END PK213_IMPORT_PORTAL65_RZD;
/
CREATE OR REPLACE PACKAGE BODY PK213_IMPORT_PORTAL65_RZD
IS
...
-- ------------------------------------------------------------------- --
-- Импорт заказов по списку по данным В.Малиновского
-- ------------------------------------------------------------------- --
PROCEDURE Import_orders
IS
...
BEGIN
    ...
    /*
    -- заполняем таблицу данными В.Малиновского
    INSERT INTO PK213_PINDB_ORDERS_T PO (
        CONTRACT_NO, ACCOUNT_NO, ORDER_NO, BRM_ORDER_NO,
        RATE_PLAN, CURRENCY, CURRENCY_SECONDARY, BRM_CURRENCY_ID, 
        SERVICE_NAME, BRM_SERVICE_ID, BRM_CHARGE_TYPE,
        BRM_TIMEZONE, CYCLE_START_T, CYCLE_END_T
    )
    SELECT MO.CONTRACT_NO, MO.ACCOUNT_NO, MO.ORDER_NO, TRIM(MO.ORDER_NO),
           MO.RATEPLAN_NAME, 810 CURRENCY, 0 CURRENCY_SECONDARY, 810 BRM_CURRENCY_ID,
           CASE 
           WHEN MO.SERVICE = 'МГ/МН' THEN 'Услуги междугородной и международной телефонной связи'
           WHEN MO.SERVICE = '8-800' THEN 'Интеллектуальная сеть связи "Бесплатный вызов"'
           WHEN MO.SERVICE = 'Местная связь' THEN 'Местная телефонная связь'
           WHEN MO.SERVICE = 'Зоновая связь' THEN 'Внутризоновая телефонная связь'
           END SERVICE_NAME,
           CASE 
           WHEN MO.SERVICE = 'МГ/МН' THEN 1
           WHEN MO.SERVICE = '8-800' THEN 2
           WHEN MO.SERVICE = 'Местная связь' THEN 125
           WHEN MO.SERVICE = 'Зоновая связь' THEN 140
           END BRM_SERVICE_ID, 
           'USG' BRM_CHARGE_TYPE,
           MO.TIME_ZONE, MO.DATE_FROM, MO.DATE_TO 
      FROM INFRANET.MIGR_ORDERS MO
    ;
    */
    -- заказы получаем по запросу А.Ю.Гурова (29.08.2016)
    -- в данном случае, когда тебе дан конкретный перечень договоров и заказов, 
    -- нужно использовать запрос практически без дополнительных фильтров:
    --
...
BEGIN
    ...
    -- account_id проставляем для догрузки заказов на существующие л/с
    MERGE INTO PK213_PINDB_ALL_CONTRACTS_T X
    USING (
    ...
    WHEN MATCHED THEN UPDATE 
                         SET X.BRM_ACCOUNT_ID = A.ACCOUNT_ID,
                             ...
                             X.LOAD_CODE      = -2,
                             X.LOAD_STATUS    = 'ACCOUNT_NO - уже существует в BRM'
    ;
    v_count := SQL%ROWCOUNT;
...
BEGIN
    ...
    USING (
        SELECT DISTINCT X.ACCOUNT_NO, X.BRM_ACCOUNT_ID
          FROM PK213_PINDB_ALL_CONTRACTS_T X
         WHERE ( X.LOAD_CODE = c_LOAD_CODE_DBL OR  X.LOAD_CODE > 0 )
    ) X
    ON (
    ...
    -- проверяем на дублирование номеров заказов в исходном SQL-запросе
    UPDATE PK213_PINDB_ORDERS_T XO 
       SET XO.LOAD_CODE = -1, 
           XO.LOAD_STATUS = 'Дублирование номеров заказов в исходном SQL-запросе'
     WHERE XO.ROWID IN (
        SELECT RID
          FROM (
    ...
    -- проверяем на дублирование номеров заказов в BRM
    MERGE INTO PK213_PINDB_ORDERS_T XO
    USING (
        SELECT O.ORDER_NO, O.ORDER_ID
          FROM PK213_PINDB_ORDERS_T XO, ORDER_T O
         WHERE O.ORDER_NO = XO.ORDER_NO
           AND XO.LOAD_CODE IS NULL 
    ) O
    ON (
        XO.ORDER_NO = O.ORDER_NO 
    )
    WHEN MATCHED THEN UPDATE SET XO.BRM_ORDER_ID = O.ORDER_ID, 
                                 XO.LOAD_CODE = -2, 
                                 XO.LOAD_STATUS = 'Ошибка, заказ уже существует в BRM'
    ;
    v_count := SQL%ROWCOUNT;
...
--============================================================================================
-- Установить ID услуги (установлен при загрузке)
PROCEDURE Set_service_id
IS
...
BEGIN
    ...
    -- проставляем компаненты услуг
    UPDATE PK213_PINDB_ORDERS_T XO
       SET BRM_CHARGE_TYPE = 'USG',
           BRM_SUBSERVICE_ID = 
             CASE 
                WHEN XO.BRM_SERVICE_ID = 1   THEN 1 -- 'МГ-МН' 2 - подставим при формировании order_body
                WHEN XO.BRM_SERVICE_ID = 125 THEN 5 -- 'Только местный трафик'
                WHEN XO.BRM_SERVICE_ID = 140 THEN 6 -- 'Только зоновый трафик'
                WHEN XO.BRM_SERVICE_ID = 2   THEN 8 -- 'Free Call'
             END
    ...
    -- помечаем заказы с тарифом MultiPart 8-800
        ...
        -- этот вариант не работает если Малиновский дал список заказов. Он вероятно не дает в списке MP8800...      
        -- тогда честно проверяем по его таблицам
        MERGE INTO PK213_PINDB_ORDERS_T XO
        USING ( 
            SELECT DISTINCT XO.ORDER_NO PARENT_ORDER_NO 
              FROM PK213_PINDB_ORDERS_T XO
             WHERE XO.BRM_SERVICE_ID = 2 
               AND XO.ORDER_NO not LIKE '%M%'
               -- проверка существования доп частей по таблицам Малиновского
               and exists (
                 select 1 from tbl_orders@SQLMMTSDB.WORLD mo
                 where mo."order" like XO.ORDER_NO||'M%'
               )
        ) M
        ON (
            XO.ORDER_NO = M.PARENT_ORDER_NO
...
BEGIN
    ...
    FOR r_abn IN c_CTR LOOP
      ...
      BEGIN
        ...
        -- приводим КПП к правильному виду (для договоров с иностранными компаниями)
        IF LENGTH(r_abn.kpp) > 10 THEN
          v_kpp := NULL;
        ELSE
          v_kpp := r_abn.kpp;
        END IF;
        -- приводим ИНН к правильному виду (для договоров с иностранными компаниями)
        ...
        */
        ...
        IF v_count = 0 THEN
            ...
            --    ;
            --END IF;
            -- привязываем менеджера к лицевому счету (С.Макеев - 06.06.2016)
            IF r_abn.brm_sale_curator_id IS NOT NULL THEN 
                ...
                ;
            END IF;
            /* Не актуально (С.Макеев - 17.06.2016)
      ...
      BEGIN
        ...
        IF v_count = 0 THEN
            ...
            INSERT INTO ORDER_T (
               ...
               NULL, -- проставим позже, при отдельном импорте
               NVL(r_ord.order_date, r_ord.cycle_start_t),
               CASE 
                 WHEN r_ord.cycle_end_t IS NULL THEN TO_DATE('01.01.2050','dd.mm.yyyy')
                 WHEN r_ord.cycle_end_t > TO_DATE('01.01.2000','dd.mm.yyyy') THEN r_ord.cycle_end_t-1/86400
                 ELSE r_ord.cycle_end_t
               END,
               SYSDATE, SYSDATE, NULL, 
               'импортировано из Portal65-RZD '|| TO_CHAR(SYSDATE,'dd.mm.yyyy')
            );
            /*
            -- Добавляем информацию о точках подключения (нет информации)
        ...
        IF r_ord.brm_subservice_id    = 1 THEN -- 'МГ-МН' формируем 2 компонента МГ(1) и МН(2)
            -- компомента 'МГ' - cdzpm
        ...
        ELSIF r_ord.brm_subservice_id = 5 OR   -- местное соединение
              r_ord.brm_subservice_id = 6 OR   -- внутризоновое присоединение
              r_ord.brm_subservice_id = 8      -- 8-800 free call 
            THEN 
            --
            INSERT INTO ORDER_BODY_T(
                ORDER_BODY_ID, ORDER_ID, SUBSERVICE_ID, CHARGE_TYPE, 
        ...
        ELSIF r_ord.brm_subservice_id IS NULL THEN
           NULL; -- SERVICE_ID = 142, 'Присоединение и пропуск трафика местного уровня'
                 -- строки ORDER_BODY_T формируются 'Динамически, из названия'
                 -- при загрузке трафика
        END IF;
