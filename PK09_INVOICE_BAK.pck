...
IS
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Расчитать позицию счета-фактуры, для регулярного счета 'B'
    -- item-ы распределяются на invoice_item-ы по alias-ам услуг на заказе
    -- Возвращает:
    --   - положительное - ID invoice_item, 
    --   - при ошибке выставляет исключение
    PROCEDURE Calc_inv_item_by_ord_alias (
                   p_inv_item_no   IN OUT INTEGER, -- номер строки в счете фактуре
                   p_bill_id       IN INTEGER,   -- ID позиции счета
                   p_rep_period_id IN INTEGER,   -- ID отчетного периода счета
                   p_service_id    IN INTEGER,   -- ID услуги
                   p_ord_srv_name  IN VARCHAR2,  -- имя услуги установленное для заказа
                   p_inv_item_name IN VARCHAR2,  -- имя строки в счете фактуре
                   p_vat           IN NUMBER,    -- ставка налога в процентах
                   p_bill_type     IN VARCHAR2   -- тип счета
               );               
...
PROCEDURE Calc_inv_item_std (
               ...
               p_rep_period_id IN INTEGER,   -- ID отчетного периода счета
               p_service_id    IN INTEGER,   -- ID услуги
               p_inv_item_name IN VARCHAR2,  -- имя строки в счете фактуре, заданное от л/с и выше
               p_vat           IN NUMBER,    -- ставка налога в процентах
               p_bill_type     IN VARCHAR2   -- тип счета
           ) 
IS
...
IS
    v_prcName       CONSTANT VARCHAR2(30) := 'Calc_inv_item_by_ord_alias';
    v_inv_item_id   INTEGER;
    v_count         INTEGER;
    --
BEGIN
        ...
        FROM (
          ...
          FROM ITEM_T I, ORDER_T O, PERIOD_T P
           WHERE I.BILL_ID       = p_bill_id
             ...
             AND P.PERIOD_ID     = I.REP_PERIOD_ID
             AND (
                 O.SERVICE_ALIAS = p_ord_srv_name 
                 OR (
                    O.SERVICE_ALIAS IS NULL AND p_ord_srv_name IS NULL
                 )
             )
          GROUP BY TAX_INCL
      )
    ...
    IF v_count = 1 THEN
        ...
        MERGE INTO ITEM_T I
        USING (
          SELECT I.REP_PERIOD_ID, I.ITEM_ID, v_inv_item_id INV_ITEM_ID
            FROM ITEM_T I, ORDER_T O
             WHERE I.BILL_ID       = p_bill_id
               AND I.REP_PERIOD_ID = p_rep_period_id
               AND I.ORDER_ID      = O.ORDER_ID
        ...
        ON (
           I.REP_PERIOD_ID = IO.REP_PERIOD_ID AND 
           I.ITEM_ID = IO.ITEM_ID
        )
        WHEN MATCHED THEN UPDATE SET I.INV_ITEM_ID = IO.INV_ITEM_ID;
...
IS
    ...
    v_ord_srv_name  INVOICE_ITEM_T.INV_ITEM_NAME%TYPE;
    v_bill_type     CHAR(1);
    v_date_from     DATE;
...
BEGIN
    ...
    THEN
        ...
        OPEN c_items FOR
          SELECT I.SERVICE_ID, NULL SERVICE_ALIAS, 
                 TRUNC(I.DATE_FROM,'mm') DATE_FROM 
            FROM ITEM_T I
           WHERE BILL_ID = p_bill_id
             AND I.REP_PERIOD_ID = p_rep_period_id
           GROUP BY I.SERVICE_ID, TRUNC(I.DATE_FROM,'mm')
           ORDER BY 1,2
        ;
    ELSIF v_inv_rule_id = 7708  THEN
        -- группировка по наименованиям услуг в заказе: ORDER_T.SERVICE_ALIAS
        OPEN c_items FOR
    ...
    --
    LOOP
        FETCH c_items INTO v_service_id, v_ord_srv_name, v_date_from;
        EXIT WHEN c_items%NOTFOUND;

        -- получаем имя позиции счета фактуры
        IF v_ord_srv_name IS NULL THEN
        ...
        ELSE
            v_inv_item_name := v_ord_srv_name;
        END IF;
        ...
        ELSIF v_inv_rule_id = 7708 THEN --   группировка по наименованиям услуг в заказе: ORDER_T.SERVICE_ALIAS
            Calc_inv_item_by_ord_alias (
                p_inv_item_no   => v_count,        -- номер строки в счете фактуре
                p_bill_id       => p_bill_id,      -- ID позиции счета
                p_rep_period_id => p_rep_period_id,-- ID отчетного периода счета
                p_service_id    => v_service_id,   -- ID услуги
                p_ord_srv_name  => v_ord_srv_name, -- имя услуги установленное для заказа
                p_inv_item_name => v_inv_item_name,-- имя строки в счете фактуре
                p_vat           => v_vat,          -- ставка налога в процентах
