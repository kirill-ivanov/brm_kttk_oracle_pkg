CREATE OR REPLACE PACKAGE PK111_TARIFFING_OLD
IS

    gc_PkgName       CONSTANT varchar2(36) := 'PK111_TARIFFING_OLD';
...
END PK111_TARIFFING_OLD;
/
CREATE OR REPLACE PACKAGE BODY PK111_TARIFFING_OLD
IS
   ...
   -----
    ...
    TYPE t_Varch IS TABLE OF varchar2(128) INDEX BY varchar2(20);
    ...
    LOOP  -- перебор дней в заданном периоде
       ...
       -- привязываем соединения     
        IF p_Data_Type IN ('MMTS','MMTS_FLT','AG_MMTS') THEN
            ...
            Pk120_Bind_Clients_Old.Bind_Order_A(p_Data_Type  => p_Data_Type, 
                                            p_Day        => l_Date_From,
                                            p_Pivot_Tbl  => NULL,
        ...
        ELSIF p_Data_Type IN ('ZONE','AG_ZONE','ZONE_FLT') THEN
            ...
            Pk120_Bind_Clients_Old.Bind_Zone_Order_A(p_Data_Type  => p_Data_Type, 
                                                 p_Day        => l_Date_From,
                                                 p_Pivot_Tbl  => NULL,
                                                 p_Result_Tbl => l_Tmp_Table,
                                                 p_Upd_CDR    => FALSE,
        ...
        ELSIF p_Data_Type IN ('SPBCLN') THEN
            ...
            pk120_Bind_Clients_Old.Bind_SPb_Clients(
                                              p_Day        => l_Date_From,
                                              p_Pivot_Tbl  => NULL,
                                              p_Result_Tbl => l_Tmp_Table,
                                              p_Upd_CDR    => FALSE,
       ...
       -- тарификация 
        ...
        PK111_TARIFFING_OLD.Load_BDR_Cl_A(p_Data_Type    => p_Data_Type,
        ...
        IF p_Data_Type IN ('MMTS','ZONE') THEN                                     
           -- для сети MMTS считаем сразу агентские договора                              
            PK111_TARIFFING_OLD.Load_BDR_Cl_A(p_Data_Type    => (CASE p_Data_Type 
        ...
        IF p_Test_BDR_Table IS NULL THEN
           ...
           -- обновляем данные о л/с в таблице CDR-ов
            l_Count := Pk120_Bind_Clients_Old.Update_Order_A_CDR(p_Data_Type  => p_Data_Type,
                                                             p_Data_Table => l_Tmp_Table,
                                                             p_Day        => l_Date_From,
                                                             p_Id_Log     => NULL
                                                            ); 
            ...
            Pk01_Syslog.Write_Msg(p_Msg => 'Update cdr. '|| p_Data_Type || ', Day: ' || TO_CHAR(l_Date_From,'dd.mm.yyyy') ||
                                  ...
                                  p_Src => 'Pk120_Bind_Clients_Old.Update_Order_A_CDR');                                                                         
...
BEGIN
   ...
   -- Проверка на задвой BDR-ов в таблице для мн,мг и зонов. соед.
    ...
    l_SQL :=   
       ...
       '       FROM TABLE(CAST(PK111_TARIFFING_OLD.Trf_Cl_A_Table( ' || CHR(13) ||
...
BEGIN
    ...
    l_Result := Get_Dir_Full(p_PH_Num      => p_PH_Num,
                             p_Prefix      => p_Prefix,
                             p_Z_Id        => l_Z_Id,
                             p_Z_Name      => p_Z_Name,
...
IS
    ...
    l_Prefix_A varchar2(15);
    l_Z_Id_B   number;
    l_Z_Name_B varchar2(255);                                
    l_Prefix_B varchar2(15);   
    l_Type_B   varchar2(1);
...
BEGIN
    ...
    BEGIN
        ...
        ELSIF l_Result = pk00_const.c_CALENDAR_HOLIDAY_ID THEN -- праздник
            l_Result := 1;
        ELSE
           -- какая-то фигня. Идем по стандартной схеме дальше 
            l_Result := NULL; 
        END IF;
    ...
    IF l_Result IS NULL OR l_Result = 0 THEN
        ...
        IF TRIM(TO_CHAR(p_Call_Time,'DAY')) IN ('SATURDAY','SUNDAY') THEN
        ...
        ELSE
            ...
            BEGIN
               -- проверяем сначала нет ли индивидуальных значений рабочего времени на ТП 
                ...
                THEN
                    l_Result := 0; -- рабочее время
                ELSE
                    l_Result := 1; -- льготное время
                END IF;
            
            EXCEPTION
                WHEN no_data_found THEN
                  -- индивидуальных значений льготного времени на ТП нет, идем по стандартной схеме
                    BEGIN
...
BEGIN
    ...
    ELSE
        ...
        ret_rec.Abn_A := pin.Norm_Ph_Number('KTTK_MMTS',pr_Call.Abn_A,'A');--PK111_TARIFFING_OLD.Normalize_A(pr_Call.Abn_A);
 ...
 /*   IF mdv.pck_tools.isnumber(l_Abn_B) != 1 AND l_Abn_B IS NOT NULL THEN
        l_Abn_B := REGEXP_REPLACE(l_Abn_B,'[A-Z]');
    END IF; */
    ...
    BEGIN        
      ...
      /*  -- получаем тип соединения (МН, МГ и т.д.)
        ...
        ELSE    
            -- определяем тип соединения (МН, МГ и т.д.)                     
            ret_rec.call_type := Get_Trf_Type(p_Phone_A => l_Abn_A,
                                              p_Phone_B => l_Abn_B);
        END IF;     

        -- определяем службу        
        IF ret_rec.Call_Type = 8 THEN
            ret_rec.SubService_Id := pk00_const.c_SUBSRV_MN;
        ELSIF ret_rec.Call_Type = 7 THEN   
            ret_rec.SubService_Id := pk00_const.c_SUBSRV_MG;
        ELSIF ret_rec.Call_Type = 9 THEN   
            ret_rec.SubService_Id := pk00_const.c_SUBSRV_ZONE; 
        ELSIF ret_rec.Call_Type = 0 THEN    
            ret_rec.SubService_Id := pk00_const.c_SUBSRV_LOCAL;
        ...
        IF ret_rec.Call_Type IN (0,9) THEN 
           -- если звонок местный или зоновый, то пишем ошибку
            RAISE CALL_TYPE_ERR;
        END IF;*/
       ...
       -- (тарифы МГ,МН и freecall)
        ...
        BEGIN
           --- Поиск тарифного плана в схеме тарифов               
            SELECT h.trf_id, h.round_v_id, h.unpaid_seconds, h.zmdl_id, h.is_8800_MP
              INTO ret_rec.trf_id, l_Round_Id, l_Unpaid_Sec, ret_rec.zmdl_id, l_8800_MP 
              FROM tariff_ph.d41_trf_header h
             WHERE h.code = ret_rec.Trf_Code; 
                    
        EXCEPTION
            WHEN no_data_found THEN
                RAISE TARIFF_NOT_FOUND;
            WHEN too_many_rows THEN
                RAISE TARIFF_NOT_UNIQUE;
        END;        
       ...
       -- 2. определяем направление      
        IF l_8800_MP = 'Y' THEN
            ...
            ret_rec.BDR_Status := Get_Zone(p_Trf_Id      => ret_rec.trf_id,    -- in
        ...
        IF ret_rec.Init_Z_ID IS NULL THEN
           ...
           /* l_Result := Get_ABC_Pref(p_PH_Num      => l_Abn_A,     -- in
            ...
            ret_rec.init_z_id := Get_Dir(p_PH_Num => l_Abn_A,     -- in
                                         p_Prefix => ret_rec.prefix_a,  -- out
                                         p_Z_Name => ret_rec.Init_Z_Name -- out
                                        );
   ...
   -- если звонки на 8800, то переподстановка номеров (так внесены тарифы для совместимости с обычными тарифами)  
    ...
    BEGIN        
       ...
       --- Поиск тарифного плана в схеме тарифов               
        ...
        IF l_Result NOT IN (t_Subservice('LOCAL'), t_Subservice('ZONE')) THEN
...
BEGIN
    ...
    LOOP
        ...
        IF l_cur.data_type IN ('MMTS','ZONE') THEN
            ...
            EXCEPTION
                WHEN no_data_found THEN
                    ...
                    l_Smeta := 0; -- флаг расчета по схеме TTK_SMETA
        ...
        IF l_cur.data_type IN ('MMTS','ZONE') AND l_Smeta = pk00_const.с_RATESYS_CL_OPR_ID THEN
        ...
        ELSIF (
                (l_cur.data_type = 'MMTS' AND NVL(l_Smeta,-1) != pk00_const.с_RATESYS_CL_OPR_ID)
                  OR                
        ...
        ELSIF (
               (l_cur.data_type = 'ZONE' AND NVL(l_Smeta,-1) != pk00_const.с_RATESYS_CL_OPR_ID)
                  OR                
...
BEGIN
   ...
   -- инициализация массива с типами сетей
    ...
    FOR l_cur IN (SELECT switch_code, switch_name
                    FROM SWITCH_T 
                 )
    LOOP
        t_Switch(l_cur.switch_code) := l_cur.switch_name; 
    END LOOP;        
    ...
    FOR l_cur IN (/*SELECT KEY, NAME REASON 
                    FROM DICTIONARY_T 
                   WHERE PARENT_ID = 72*/
                   SELECT switch_name, code, term_reason 
                     FROM SW_TERM_REASON_T   
                 )
    LOOP
       -- t_Reason_Name(l_cur.KEY) := l_cur.reason;
        t_Term_Reason(l_cur.switch_name)(TO_CHAR(l_cur.code)) := l_cur.term_reason;         
    END LOOP;        
