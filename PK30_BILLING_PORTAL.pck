CREATE OR REPLACE PACKAGE PK30_BILLING_PORTAL
IS
    --
    -- Пакет для поддержки процесса выставления счетов и закрытия периода
    -- биллинга PORTAL 6.5 ( старого биллинга )
    -- для клиентов Юр.лиц
    -- Закрываются только счета стоящие в очереди: BILLING_QUEUE_T
    --
    -- ==============================================================================
    c_PkgName   constant varchar2(30) := 'PK30_BILLING_PORTAL';
    -- ==============================================================================
    -- В Ы Б О Р   Б И Л Л И Н Г А
    -- ------------------------------------------------------------------------- --
    -- заполнить очередь на выставление всех счетов
...
END PK30_BILLING_PORTAL;
/
CREATE OR REPLACE PACKAGE BODY PK30_BILLING_PORTAL
IS
      ...
      FROM (
        SELECT B.BILL_ID, B.ACCOUNT_ID, A.BILLING_ID, AP.PROFILE_ID, 
         ...
         WHERE B.REP_PERIOD_ID = p_period_id
           ...
           AND A.BILLING_ID    = Pk00_Const.c_BILLING_OLD
...
BEGIN
    ...
    -- BRM_KTTK, включая клиентов импортированных из биллинга ММТС "Микротест"
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    Mark_bills( v_period_id );
    
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Начисление абонентской платы и доплаты до минимальной суммы
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    PK30_BILLING_BASE.Charge_fixrates( v_period_id );

    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Формирование счетов
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    PK30_BILLING_BASE.Make_bills(v_period_id);
    
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Финальные процедуры (для ускорения выполним оптом для всех биллингов)
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    --PK30_BILLING_BASE.Close_period(v_period_id);
