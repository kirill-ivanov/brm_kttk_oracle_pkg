...
BEGIN
    ...
    USING (
      ...
      /*
      --------------------------------------------------------------------------
      SELECT I.ITEM_ID, I.REP_PERIOD_ID, 
             CASE 
               WHEN B.BILL_TYPE NOT IN ( Pk00_Const.c_BILL_TYPE_CRD, 
                                         Pk00_Const.c_BILL_TYPE_DBT,
                                         Pk00_Const.c_BILL_TYPE_OLD 
                                       ) THEN
                 -- конвертация на дату счета
                 I.ITEM_TOTAL * Currency_rate(
                                  p_item_currency_id => I.ITEM_CURRENCY_ID,
                                  p_bill_currency_id => B.CURRENCY_ID,
                                  p_date_rate        => B.BILL_DATE)
               -- -------------------------------------------------------------
--
--               -- пока так
--               WHEN B.BILL_TYPE IN ( Pk00_Const.c_BILL_TYPE_DBT,
...
--                                  p_item_currency_id => I.ITEM_CURRENCY_ID,
--                                  p_bill_currency_id => B.CURRENCY_ID,
--                                  p_date_rate        => TRUNC(ADD_MONTHS(I.DATE_TO,1)-1))
--
               -- -------------------------------------------------------------                   
               WHEN B.BILL_TYPE = Pk00_Const.c_BILL_TYPE_OLD THEN
                 -- конвертация на дату сторнированного счета (временно)
                 I.ITEM_TOTAL * Currency_rate(
                                  p_item_currency_id => I.ITEM_CURRENCY_ID,
                                  p_bill_currency_id => B.CURRENCY_ID,
                                  p_date_rate        => TRUNC(ADD_MONTHS(I.DATE_TO,1)-1))
    ...
    */
    ...
    WHEN MATCHED THEN UPDATE SET I.BILL_TOTAL = ROUND(IQ.BILL_TOTAL,2),
    ...
    USING (
      SELECT  DISTINCT
              ITEM_ID, REP_PERIOD_ID, ITEM_CURRENCY_RATE,
             (ITEM_TOTAL * ITEM_CURRENCY_RATE) BILL_TOTAL
        FROM (
          SELECT I.ITEM_ID, I.REP_PERIOD_ID, I.ITEM_TOTAL, 
                 -- конвертация на дату сторнированного счета
            ...
            FROM (
                  SELECT B.BILL_ID, B.REP_PERIOD_ID, B.CURRENCY_ID,
                         F.BILL_DATE FIRST_DATE 
                    FROM BILL_T B, BILLING_QUEUE_T Q, BILL_T F
                   WHERE Q.BILL_ID         = B.BILL_ID
                     AND Q.REP_PERIOD_ID   = B.REP_PERIOD_ID
                     AND F.ACCOUNT_ID      = Q.ACCOUNT_ID
                     AND CONNECT_BY_ISLEAF = 1
                     AND B.BILL_TYPE IN ( Pk00_Const.c_BILL_TYPE_DBT,
                                          Pk00_Const.c_BILL_TYPE_ADS)
                  CONNECT BY NOCYCLE PRIOR F.PREV_BILL_ID = F.BILL_ID 
                                 AND PRIOR F.PREV_BILL_PERIOD_ID = F.REP_PERIOD_ID
                  START WITH F.REP_PERIOD_ID = B.REP_PERIOD_ID 
    ...
    */
            ...
            SELECT 
                   CASE
                      WHEN I.TAX_INCL = PK00_CONST.c_RATEPLAN_TAX_INCL THEN
                        (I.BILL_TOTAL - PK09_INVOICE.ALLOCATE_TAX(I.BILL_TOTAL, AP.VAT))
                   ...
                   CASE
                      WHEN I.TAX_INCL = PK00_CONST.c_RATEPLAN_TAX_INCL THEN
                        (PK09_INVOICE.ALLOCATE_TAX(I.BILL_TOTAL, AP.VAT))
                      WHEN I.TAX_INCL = PK00_CONST.c_RATEPLAN_TAX_NOT_INCL THEN
                        PK09_INVOICE.CALC_TAX(I.BILL_TOTAL, AP.VAT)
                      ELSE 
                        NULL
                   END REP_TAX,
                   Pk00_Const.c_ITEM_STATE_REАDY                    
    ...
    /*
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- проставляем статусы READY для счетов
    UPDATE BILL_T B
...
IS
BEGIN    
     RETURN  p_taxfree_total * p_tax_rate / 100;
END;
...
IS
BEGIN    
     RETURN  p_total - (p_total /(1 + p_tax_rate / 100));
END;
