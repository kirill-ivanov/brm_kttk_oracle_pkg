...
IS
    ...
    -- биллинг в который загружаем клиентов NPL, для тестирования
    c_BILLING_CCAD   CONSTANT INTEGER := 2097; -- Тестовый биллинг КСАД
    ...
    с_RATESYS_CSAD_ID CONSTANT INTEGER := 1203; -- тарификатор трафика КСАД, тариф в ORDER_BODY_T
    ...
    --============================================================================================
    -- Подготовка к загрузке заказов
    --============================================================================================
    PROCEDURE Prepare_orders;
    
    -- Загрузка информации о FLAT-RATE заказах на доступ в Интернет
    PROCEDURE Load_orders_ip0;

    -- Загрузка информации о NPL заказах 
    PROCEDURE Load_orders_npl;
    
    -- Загрузка информации о VPN заказах 
    PROCEDURE Load_orders_vpn;
    
    -- Загрузка информации о BURST заказах 
...
--============================================================================================
-- После полного тестирования переводим л/с из c_BILLING_CCAD -> Pk00_Const.c_BILLING_OLD
--============================================================================================
PROCEDURE Change_billing_id
IS
    ...
    */
    ...
    USING (
        select distinct
          s.login,
          case
            when tt.name = 'IP 0' then 'FLAT' -- простая абонентка
            else tt.type 
          end ttype
        from 
            service_t@PINDB.WORLD.world s,
            account_t@PINDB.WORLD.world a,
            account_products_t@PINDB.WORLD.world ap, 
            iprouting_t@PINDB.WORLD.world ip,
            ttc_tariffs@PINDB.WORLD.world tt,
            v_price_list_small@PINDB.WORLD.world  p
        where     
...
BEGIN
    ...
    v_count := SQL%ROWCOUNT;
    Pk01_Syslog.Write_msg('PINDB_ALL_CONTRACTS_T.SALE_CURATOR: '||v_count||' rows merged (Привязка продавцов)', c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );
    ...
    v_count := SQL%ROWCOUNT;
    Pk01_Syslog.Write_msg('PINDB_ALL_CONTRACTS_T.BILLING_CURATOR: '||v_count||' rows merged (Привязка сотрудников ДРУ)', c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );
...
BEGIN
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    UPDATE PINDB_CCAD_ORDERS_T PO SET PO.LOAD_STATUS = c_LDSTAT_DBL_ORD -- заказ уже есть в BRM
    WHERE EXISTS (
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    UPDATE PINDB_CCAD_ORDERS_T PO SET PO.LOAD_STATUS = c_LDSTAT_NOT_SRV -- услуга не найдена
    WHERE NOT EXISTS (
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    -- Проверяем все ли заказы можем загрузить, пока по тарифным планам
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    v_count := 0;
    FOR ro IN (
...
BEGIN
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    -- создаем заказы на IP каналы за абонку: IP Routing -> IP 0
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    INSERT INTO ORDER_T(
    ...
    WITH PO AS (
        SELECT DISTINCT
            ...
            CYCLE_START_T, CYCLE_END_T-1/86400 CYCLE_END_T, -- начало и окончание начисления абонплаты 
            --SMC_START_T, SMC_END_T-1/86400,     -- текущи период (фигня)
        ...
        WHERE 1=1 --IP_USAGE_RATE_PLAN IS NULL
          AND LOAD_STATUS IS NULL    -- записи прошли провеку на корректность
          -- - - - - - - - - - - - - - - - - - - - - - - - -  
          -- каналы за абонку: IP Routing -> IP 0 
          AND (RATE_PLAN LIKE 'IP Routing%' OR RATE_PLAN LIKE 'IP Burst%')
          AND IP_USAGE_RATE_PLAN = 'IP 0'
          -- - - - - - - - - - - - - - - - - - - - - - - - - 
          -- кналы NPL, только абонплата
    ...
    SELECT SQ_ORDER_ID.NEXTVAL ORDER_ID, 
           ...
           /* на заказ тарифный план не вешаем
           CASE
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    -- создаем компоненты услуг для абонентской платы
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    SELECT SQ_ORDER_ID.NEXTVAL ORDER_BODY_ID, O.ORDER_ID,
           ...
           Pk00_Const.c_RATE_RULE_ABP_STD RATE_RULE_ID, -- 2402; -- Стандартное начисление абонплаты  
...
BEGIN
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    -- создаем заказы на VPN каналы + минималку
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    INSERT INTO ORDER_T(
        ORDER_ID, EXTERNAL_ID, ORDER_NO, ACCOUNT_ID, SERVICE_ID, RATEPLAN_ID, 
        DATE_FROM, DATE_TO, CREATE_DATE, MODIFY_DATE, STATUS, NOTES
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    -- создаем компоненты услуг для расчета трафика VPN
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    INSERT INTO ORDER_BODY_T (
        ORDER_BODY_ID, ORDER_ID, SUBSERVICE_ID, CHARGE_TYPE, DATE_FROM, DATE_TO,
        RATEPLAN_ID, RATE_VALUE, RATE_RULE_ID, RATE_LEVEL_ID, TAX_INCL, QUANTITY,
    ...
    SELECT SQ_ORDER_ID.NEXTVAL ORDER_BODY_ID, O.ORDER_ID,
           Pk00_Const.c_SUBSRV_VOLUME,   -- 39; -- Тарификация по объему
           Pk00_Const.c_CHARGE_TYPE_USG, -- 'USG'
           PO.CYCLE_START_T DATE_FROM, 
           NVL(PO.CYCLE_END_T, c_MAX_DATE_TO) DATE_TO,
           NULL RATEPLAN_ID,             --  будет проставлем из АРМ - КСАД
           NULL RATE_VALUE,
           Pk00_Const.c_RATE_RULE_IP_VPN,-- 2408; -- VPN - начисление трафика
           NULL RATE_LAVEL_ID,           -- для трафика не указываем
           'N' TAX_INCL,
           NULL QUANTITY,
           SYSDATE CREATE_DATE,
           SYSDATE MODIFY_DATE,
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    -- создаем компоненты услуг для доплаты до минимального трафика
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    INSERT INTO ORDER_BODY_T (
        ORDER_BODY_ID, ORDER_ID, SUBSERVICE_ID, CHARGE_TYPE, DATE_FROM, DATE_TO,
        RATEPLAN_ID, RATE_VALUE, RATE_RULE_ID, RATE_LEVEL_ID, TAX_INCL, QUANTITY,
    ...
    SELECT
     ...
     WHERE O.ORDER_NO = PO.ORDER_NO
       AND O.ORDER_ID = OB.ORDER_ID
       AND OB.CHARGE_TYPE = Pk00_Const.c_CHARGE_TYPE_USG -- 'USG' - вешаем на трафик
       AND D.PARENT_ID(+) = 67
       AND D.NAME = TRIM(SUBSTR(LTRIM(PO.SPEED_STR), INSTR(LTRIM(PO.SPEED_STR),' ')))
       AND PO.LOAD_STATUS IS NULL
       AND PO.RATE_PLAN LIKE 'IP Routing%'
...
BEGIN
    ...
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    -- создаем компоненты услуг для расчета трафика BURST
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    INSERT INTO ORDER_BODY_T (
        ORDER_BODY_ID, ORDER_ID, SUBSERVICE_ID, CHARGE_TYPE, DATE_FROM, DATE_TO,
        RATEPLAN_ID, RATE_VALUE, RATE_RULE_ID, RATE_LEVEL_ID, TAX_INCL, QUANTITY,
    ...
    SELECT SQ_ORDER_ID.NEXTVAL ORDER_BODY_ID, O.ORDER_ID,
           Pk00_Const.c_SUBSRV_BURST,       -- 40; -- Тарификация по методу BURST
           Pk00_Const.c_CHARGE_TYPE_USG,    -- 'USG'
           PO.CYCLE_START_T DATE_FROM, 
           NVL(PO.CYCLE_END_T, c_MAX_DATE_TO) DATE_TO,
           NULL RATEPLAN_ID,                -- Тариф будет подтянут из АРМ КСАД 
           NULL RATE_VALUE,
