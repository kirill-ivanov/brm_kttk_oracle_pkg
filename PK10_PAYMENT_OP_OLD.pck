CREATE OR REPLACE PACKAGE PK10_PAYMENT_OP_OLD
IS
    --
...
END PK10_PAYMENT_OP_OLD;
/
CREATE OR REPLACE PACKAGE BODY PK10_PAYMENT_OP_OLD
IS
...
BEGIN
    ...
    -- удалить все операции с платежем (Хромых Ю.С.)
    DELETE FROM payment_operation_t p
     WHERE p.src_payment_id    = p_payment_id 
       AND p.src_rep_period_id = p_pay_period_id;
...
BEGIN
    ...
    INSERT INTO PAYMENT_T (
        ...
        r_payment.REFUND, NULL, nvl(p_notes, r_payment.NOTES), r_payment.EXTERNAL_ID
    );
