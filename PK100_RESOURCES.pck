...
IS
    ...
    PROCEDURE Load_Oper_Service_Ph;
    ...
    SELECT order_id, NAME, code
      FROM (
            SELECT pino.order_id, rp.NAME, rp.code, -- RP434044D6
                   row_number() OVER (PARTITION BY pino.order_id ORDER BY pin.u2d@mmtdb.world(pae.valid_to) DESC) rn
              FROM pin.account_t@mmtdb.world a, 
                   pin.profile_t@mmtdb.world p, 
                   pin.profile_acct_extrating_data_t@mmtdb.world pae,
                   integrate.ifw_rateplan@mmtdb.world rp,
                   pin.account_t pina,
                   pin.order_t pino,
                   pin.service_t pins
    ...
    SELECT order_id, NAME, code
      FROM (
            SELECT -- COUNT(1) -- 248272
                    ...
                    row_number() OVER (PARTITION BY order_id ORDER BY NVL(pin.U2D@mmtdb.world(pp.usage_end_t), gc_MaxDate) DESC) rn
              FROM  pin.account_t@mmtdb.world a,
                    pin.purchased_product_t@mmtdb.world pp, 
                    pin.product_usage_map_t@mmtdb.world pum,
                    pin.service_t@mmtdb.world s,
                    integrate.ifw_rateplan@mmtdb.worldrp, -- описатель rateplan связка с PIN.PRODUCT_USAGE_MAP_T
                    pin.account_t pina,
                    pin.order_t pino,
                    pin.service_t pins
            WHERE 1=1
...
BEGIN
    ...
    INSERT INTO TMP_ORDER_PHONES_T(
        ...
        SELECT pino.order_id, sal.NAME ph_num, 
               TRUNC(NVL(pin.u2d@mmtdb.world(s.effective_t), pin.u2d@mmtdb.world(s.created_t))) 
          ...
          FROM pin.account_t@mmtdb.world a,
               pin.service_t@mmtdb.world s, 
               pin.service_alias_list_t@mmtdb.world sal,
               pin.au_service_t@mmtdb.world aus,
               pin.account_t pina,
               pin.order_t pino,
               pin.service_t pins
         ...
         GROUP BY pino.order_id, sal.NAME, 
                  NVL(pin.u2d@mmtdb.world(s.effective_t),pin.u2d@mmtdb.world(s.created_t)),
                  s.poid_id0, s.poid_type, s.status;
    ...
    SELECT pino.order_id, MIN(sal.NAME) ph_num, 
      ...
      FROM pin.account_t@mmtdb.world a,
           pin.service_t@mmtdb.world s, 
           pin.au_service_alias_list_t@mmtdb.world sal,
           pin.au_service_t@mmtdb.world aus,
           pin.account_t pina,
           pin.order_t pino,
           pin.service_t pins
    ...
    SELECT --+ use_hash(t)
           t.order_id,
           sal.NAME                                 ph_num,
           TRUNC(NVL(pin.u2d@mmtdb.world(s.effective_t),
                     pin.u2d@mmtdb.world(s.created_t)))   
               + DECODE(s.status, gc_Active, 0,
           ...
           pin.u2d@mmtdb.world(s.created_t)               au_created_t,           
           2                                        rec_source
           --DECODE(a.business_type,1,'Ф',2,'Ю')      account_type
      FROM TMP_ORDER_PHONES_T t,
           pin.au_service_alias_list_t@mmtdb.world sal,
           pin.au_service_t@mmtdb.world s
     WHERE 1=1
 ...
 /*   UPDATE TMP_ORDER_PHONES_T tt
     ...
     WHERE tt.rowid IN (
                   SELECT t.rowid
                     FROM pin.profile_t@mmtdb.world p, 
                          pin.profile_acct_extrating_data_t@mmtdb.world pae,
...
BEGIN
   ...
   --   
    ...
    INSERT INTO TMP_ORDER_PHONES_T(
               ...
               not_transform)
        SELECT pino.order_id, sal.NAME ph_num, 
               GREATEST(TRUNC(NVL(pin.u2d@mmtdb.world(s.effective_t), pin.u2d@mmtdb.world(s.created_t))) 
                         + DECODE(s.status, gc_Active, 0,
                                            gc_NotActive, 1-1/86400),
          ...
          FROM pin.order_t pino,
               pin.profile_contract_info_t@mmtdb.world pci,
               pin.profile_t@mmtdb.world p,
               pin.service_t@mmtdb.world s, 
               pin.service_alias_list_t@mmtdb.world sal,
               pin.au_service_t@mmtdb.world aus
   ...
   -- аудит profile
    INSERT INTO TMP_ORDER_PHONES_T(
        ...
        SELECT pino.order_id, sal.NAME ph_num, 
               ...
               -- DECODE(a.business_type,1,'Ф',2,'Ю') 
          FROM pin.order_t pino,
               pin.au_profile_contract_info_t@mmtdb.world pci,
               pin.au_profile_t@mmtdb.world p,
               pin.service_t@mmtdb.world s, 
               pin.service_alias_list_t@mmtdb.world sal,
               pin.au_service_t@mmtdb.world aus
         WHERE pino.order_no = pci.order_num
  ...
  --  Pk01_Syslog.write_Msg( p_Msg => 'Step2: ' || TO_CHAR(SYSDATE,'dd.mm.yyyy hh24:mi:ss'),    
   --                        p_Src => c_PkgName||'.'||c_prcName );
    ...
    SELECT pino.order_id, MIN(sal.NAME) ph_num,
           pino.date_from,
           LEAST(TRUNC(NVL(pin.u2d@mmtdb.world(s.effective_t),pin.u2d@mmtdb.world(s.created_t))) 
         ...
         -- до 03.06.2014  
          /* GREATEST(TRUNC(NVL(pin.u2d@mmtdb.world(s.effective_t),pin.u2d@mmtdb.world(s.created_t))) 
           ...
           NVL(pin.u2d@mmtdb.world(s.effective_t),pin.u2d@mmtdb.world(s.created_t)), --gc_MaxDate, 
           1 SRC,
           0 not_transform
           -- DECODE(a.business_type,1,'Ф',2,'Ю') 
      FROM pin.order_t pino,
   ...
   -- удаляем те записи, где получилось что дата начала больше даты конца 
    DELETE
      FROM TMP_ORDER_PHONES_T
     WHERE date_FROM > date_to;
  ...
  --                         p_Src => c_PkgName||'.'||c_prcName );
    ...
    SELECT pino.order_id, MIN(sal.NAME) ph_num, 
     ...
     WHERE pino.order_no = pci.order_num
       ...
       AND aus.poid_id0 = sal.obj_id0          
       AND NOT EXISTS (SELECT 1
                         FROM pin.service_alias_list_t@mmtdb.world sal
  ...
  --                         p_Src => c_PkgName||'.'||c_prcName );   
   ...
   -- COMMIT;
    ...
    USING (
            SELECT t.rd,
                   t.date_from, next_df,
                   DECODE(srv_status, gc_Closed, date_to, NVL(t.next_df-1/86400, gc_MaxDate)) new_date_to, -- c 04.07.2014
                   --NVL(t.next_df-1/86400, gc_MaxDate) new_date_to,
                   date_to, 
                   --next_status, 
                   --srv_status_to, 
 ...
 --                          p_Src => c_PkgName||'.'||c_prcName );   
    ...
    WHERE not_transform != 1
      AND rowid IN (  
                ...
                --   AND t1.srv_poid_type = t2.srv_poid_type  -- удалено 04.07.2014  
                   AND t1.phone_number = t2.phone_number
                   AND t2.rec_source != 1
                   AND t1.rec_source <= t2.rec_source -- по приоритету при прочих равных остаются открытые
                   AND t1.au_created_t > t2.au_created_t -- и с наиболее свежей датой изменения
           ...
           SELECT rd, new_date_to
                 FROM (
                        SELECT t.rd,
                          ...
                          FROM (
                                SELECT --r.ph_num, r.srv_poid_type,
                                       lag(r.date_from) OVER (PARTITION BY r.phone_number 
                                                                          --, r.srv_poid_type -- удалено 04.07.2014
                                                                  ORDER BY r.date_to DESC, r.rec_source, r.au_created_t DESC) next_df, -- c 04.07.2014        
                                                                  --ORDER BY r.rec_source, r.au_created_t DESC, r.date_from DESC) next_df,
                                       r.date_from,
                                       r.date_to,
                                       r.ROWID rd
    ...
    DELETE FROM PIN.ORDER_PHONES_T p
     WHERE p.order_id NOT IN (SELECT o.order_id
       ...
       AND NOT EXISTS (SELECT 1
                         FROM PIN.TMP_ORDER_PHONES_T t 
                        WHERE t.order_id     = p.order_id
                          AND t.phone_number = NVL(p.phone_number,'00000') 
                          AND t.date_from    = NVL(p.date_from, gc_MinDate) 
                          AND t.date_to      = NVL(p.date_to, gc_MaxDate)
                      );
...
BEGIN
  ...
  -- ++++++++++++++++++++++++++++++++++++      
   ...
   -- удаляем данные из item_t      
    DELETE FROM detail_mmts_t_jur d
    ...
    Pk01_Syslog.write_Msg(p_Msg => 'удалено из detail_mmts_t_jur: ' || TO_CHAR(SQL%ROWCOUNT), 
                          p_Src => c_PkgName||'.'||c_prcName ); 
                          
    DELETE FROM detail_mmts_t_fiz d
     WHERE d.item_id IN (SELECT i.item_id
                           FROM account_t a,
                                order_t o,
                                item_t i
    ...
    Pk01_Syslog.write_Msg(p_Msg => 'удалено из detail_mmts_t_fiz: ' || TO_CHAR(SQL%ROWCOUNT), 
                          p_Src => c_PkgName||'.'||c_prcName );     
   ...
   -- удаляем заказы
    ...
    FOR l_cur IN (
                    SELECT --a.account_id,
                           ...
                           TRUNC (pin.u2d@mmtdb.world(MIN(p.effective_t))) order_date
                      FROM pin.account_t@mmtdb.world ac,
                           pin.account_t@mmtdb.world ap,
                           pin.billinfo_t@mmtdb.world bic,
                           pin.billinfo_t@mmtdb.world bip,
                           pin.service_t@mmtdb.world s,
                           pin.profile_t@mmtdb.world p,
                           pin.profile_contract_info_t@mmtdb.world pci
                           --pin.account_t a
                                ...
                                SELECT code, date_from, date_to
                                  ...
                                  FROM (
                                        SELECT rp.code,
                                          ...
                                          FROM pin.purchased_product_t@mmtdb.world pp, 
                                               pin.product_usage_map_t@mmtdb.world pum,
                                               pin.service_t@mmtdb.world s,
                                               integrate.ifw_rateplan@mmtdb.world rp -- описатель rateplan связка с PIN.PRODUCT_USAGE_MAP_T
                                         WHERE 1=1
    ...
    FOR l_cur IN (
                    SELECT ap.account_no,
                           ...
                           ac.poid_id0   acc_ch_id,
                           pci.order_num order_no,
                           TRUNC (pin.u2d@mmtdb.world(MIN(p.effective_t))) order_date_from,
                           TRUNC (pin.u2d@mmtdb.world(MIN(ac.last_status_t))) + 
...
BEGIN
    ...
    SELECT ap.account_no,
      ...
      FROM pin.account_t@mmtdb.world ac,
           ...
           pin.profile_t@mmtdb.world p,
           pin.profile_contract_info_t@mmtdb.world pci,
           pin.au_profile_t@mmtdb.world aup 
     WHERE ac.business_type = 1
       AND ac.status != 10103
   ...
   -- загружаем историю по действующим          
    ...
    SELECT o.account_no,
      ...
      FROM TMP05_ORDER o,
           pin.au_profile_t@mmtdb.world aup,
           pin.au_profile_contract_info_t@mmtdb.world apc
    ...
    SELECT ap.account_no,
     ...
     WHERE ac.business_type = 1
       ...
       AND (EXISTS (SELECT 1
             ...
             OR
            EXISTS (SELECT 1
                      FROM pin.au_service_t@mmtdb.world aus,
                           pin.au_service_alias_list_t@mmtdb.world sal
                                ...
                                (SELECT code, date_from, date_to
                                  FROM (
                                        SELECT pp.account_obj_id0,
                                          ...
                                          FROM TMP05_ORDER o,
                                               pin.au_purchased_product_t@mmtdb.world pp, 
                                               pin.product_usage_map_t@mmtdb.world pum,
                                               pin.service_t@mmtdb.world s
                                         WHERE 1=1
...
BEGIN
   ...
   -- загрузка промежуточных данных в mmtdb
    mdv_adm.pk02_export_p.exp_subs_info@mmtdb.world;     
   ...
   -- экспорт данных
    FOR l_cur IN (
       SELECT ACCOUNT_NO, 
              ...
              EXT_SOURCE, EXT_ID, 
              ACCOUNT_STATUS  
         FROM MDV_ADM.P_SUBS_INFO_T@mmtdb.world 
...
PROCEDURE Load_Oper_Service_Ph
IS
    c_prcName CONSTANT varchar2(32) := 'Load_Oper_Service_Ph'; 
