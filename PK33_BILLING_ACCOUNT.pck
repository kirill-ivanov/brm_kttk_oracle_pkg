...
IS
    ...
    -- ===================================================================================
    -- Удаление дебет/кредит нот из текущего или биллиногового периода
    -- ===================================================================================
    ...
    -- Расформирование фиксированных начислений, указанного периода - p_data_period_id
    ...
    -- ===================================================================== --
    -- Работа с компенсациями за простои
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    -- Расформирование компенсаций за простои
    PROCEDURE Rollback_downtimes( 
                p_bill_id   IN INTEGER,
                p_period_id IN INTEGER
              );
    -- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - --
    ...
    PROCEDURE Rollback_bill_discount(  
                p_bill_id   IN INTEGER,
                p_period_id IN INTEGER
              );
...
BEGIN
    ...
    v_task_id := PK30_BILLING_QUEUE.Open_task;
    --    
    INSERT INTO BILLING_QUEUE_T(BILL_ID, ACCOUNT_ID, TASK_ID, REP_PERIOD_ID)
    SELECT B.BILL_ID, B.ACCOUNT_ID, v_task_id, B.REP_PERIOD_ID 
...
BEGIN
    --
    Pk07_Bill_Adjust.Delete_credit_note(p_crd_bill_id, p_crd_period_id);
    --    
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    --
    Pk07_Bill_Adjust.Delete_debet_note(p_dbt_bill_id, p_dbt_period_id);
    --    
EXCEPTION
    WHEN OTHERS THEN
        Pk01_Syslog.raise_Exception('ERROR', c_PkgName||'.'||v_prcName );
...
BEGIN
    ...
    --
    -- ставим счет в очередь на обработку
    v_task_id := push_Bill(p_bill_id, p_period_id, p_data_period_id);
...
BEGIN
    ...
    --
    -- ставим счет в очередь на обработку
    v_task_id := push_Bill(p_bill_id, p_period_id, p_period_id);
...
BEGIN
    ...
    PK38_BILLING_DOWNTIME.Recharge_downtimes( v_task_id );
...
BEGIN
    ...
    PK38_BILLING_DOWNTIME.Recharge_downtime_for_debet( 
                 p_dbt_bill_id,    -- id - дебетового счета, в котором учтется скидка
                 p_dbt_period_id,  -- период дебетового счета
                 p_crd_period_id   -- период кредитового счета, для которого расчитывается скидка
              );
    --
    Pk01_Syslog.Write_msg('Stop', c_PkgName||'.'||v_prcName, Pk01_Syslog.L_info );
    --
