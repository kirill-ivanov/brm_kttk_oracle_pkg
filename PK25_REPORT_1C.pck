...
-- 5) Детальная статистика с выводом подитогов по ошибкам сравнения 
-- по счетов по договорам контрагентов в BRM и 1С
...
-- ------------------------------------------------------------------------- --
-- 6) детальное сравнение платежей по договору контрагента принятых в BRM и 1С
-- ------------------------------------------------------------------------- --
...
BEGIN
    ...
    SELECT A.ACCOUNT_ID, A.ACCOUNT_NO, C.CONTRACT_NO, CM.ERP_CODE--, A.NOTES, A.CREATE_DATE 
     ...
     WHERE A.BILLING_ID     IN (2001, 2002) -- биллинги, только центрального продукта
       ...
       AND NOT EXISTS ( -- на договоре нет л/с, заведенных позже начала периода
...
-- ------------------------------------------------------------------------- --
-- Статистика по ошибкам сравнения счетов по договорам контрагентов в BRM и 1С
-- ------------------------------------------------------------------------- --
PROCEDURE R2_BRM_vs_1C_bills(
          p_result    OUT VARCHAR2,  
          p_recordset OUT t_refc )
...
-- ------------------------------------------------------------------------- --
-- Детальная статистика с выводом подитогов по ошибкам сравнения 
-- по счетов по договорам контрагентов в BRM и 1С
-- ------------------------------------------------------------------------- --
PROCEDURE R5_BRM_vs_1C_bills(
          p_result    OUT VARCHAR2,  
     ...
     OPEN p_recordset FOR
        ...
        SELECT ERP_CODE, CONTRACT_NO, RECVD_BRM, RECVD_1C,
               ...
               CASE
                ...
                WHEN NVL(RECVD_BRM,0)!= 0 AND NVL(RECVD_1C,0) = 0 THEN 'Платежи есть в BRM и нет в 1С'
                WHEN NVL(RECVD_BRM,0) = 0 AND NVL(RECVD_1C,0)!= 0 THEN 'Платежи есть в 1С и нет в BRM'
               END ERR_MSG
          FROM RP
         ORDER BY 
               CASE
...
-- ------------------------------------------------------------------------- --
-- детальное сравнение платежей по договору контрагента принятых в BRM и 1С
-- ------------------------------------------------------------------------- --
PROCEDURE RP6_BRM_detail_pay_comp(
          p_result    OUT VARCHAR2,  
          p_recordset OUT t_refc )
