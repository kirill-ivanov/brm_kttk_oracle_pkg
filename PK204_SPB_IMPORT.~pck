...
BEGIN
    ...
    select 
        ...
        VS.ORDER_DATE, 
        VS.SERVICE_NAME,
        i2d@OLD_BILL(min(AP.CYCLE_START_T)) DATE_FROM, 
        i2d@OLD_BILL(max(AP.CYCLE_END_T))   DATE_TO,
    ...
    from account_t@OLD_BILL a 
         inner join account_products_t@OLD_BILL ap on a.poid_id0 = ap.obj_id0
         inner join account_nameinfo_t@OLD_BILL an on a.poid_id0 = an.obj_id0 and an.rec_id = 1
         inner join plan_t@OLD_BILL p on ap.plan_obj_id0 = p.poid_id0
         inner join service_t@OLD_BILL s on ap.service_obj_id0 = s.poid_id0
         inner join profile_t@OLD_BILL pr on a.poid_id0 = pr.account_obj_id0
         inner join contract_info_t@OLD_BILL ci on pr.poid_id0 = ci.obj_id0
         inner join v_all_data_serv_plus@OLD_BILL vs on S.POID_ID0 = VS.POID_ID0
         inner join clients_t@OLD_BILL cl on CI.CLIENT_ID = CL.REC_ID
         inner join payinfo_t@OLD_BILL pi on a.poid_id0 = pi.account_obj_id0
         inner join payinfo_inv_t@OLD_BILL pii on pi.poid_id0 = pii.obj_id0
